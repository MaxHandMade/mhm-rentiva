...
class VendorOnboardingControllerTest extends \WP_UnitTestCase
{
    ...
    protected function setUp(): void
    {
        ...
        update_post_meta($this->application_id, '_vendor_account_holder', 'Test Holder');
    ...
    public function test_approve_syncs_user_meta(): void
    {
        ...
        $this->assertSame('Test Holder', get_user_meta($this->user_id, '_rentiva_vendor_account_holder', true));
        $this->assertSame('Kadikoy VD', get_user_meta($this->user_id, '_rentiva_vendor_tax_office', true));
    ...
    public function test_suspend_unpublishes_vendor_vehicles(): void
    {
        // Approve vendor first.
        VendorOnboardingController::approve($this->application_id);

        // Create two vehicles owned by this vendor.
        $vehicle_1 = wp_insert_post(array(
            'post_type'   => 'vehicle',
            'post_status' => 'publish',
            'post_author' => $this->user_id,
            'post_title'  => 'Test Vehicle 1',
        ));
        update_post_meta($vehicle_1, '_mhm_vehicle_status', 'active');

        $vehicle_2 = wp_insert_post(array(
            'post_type'   => 'vehicle',
            'post_status' => 'publish',
            'post_author' => $this->user_id,
            'post_title'  => 'Test Vehicle 2',
        ));
        // Vehicle 2 has no status meta (legacy default = active).

        // Suspend the vendor.
        VendorOnboardingController::suspend($this->user_id);

        // Both vehicles should be drafted and marked inactive.
        $this->assertSame('draft', get_post_status($vehicle_1));
        $this->assertSame('inactive', get_post_meta($vehicle_1, '_mhm_vehicle_status', true));
        $this->assertSame('1', get_post_meta($vehicle_1, '_mhm_vehicle_suspended_by_vendor_ban', true));

        $this->assertSame('draft', get_post_status($vehicle_2));
        $this->assertSame('inactive', get_post_meta($vehicle_2, '_mhm_vehicle_status', true));

        // Clean up.
        wp_delete_post($vehicle_1, true);
        wp_delete_post($vehicle_2, true);
    }
