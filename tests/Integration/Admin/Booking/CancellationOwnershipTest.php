 ...
 * the same key under the old short prefix, which had no writer either. (The
 * literal is spelled out nowhere in this file on purpose -- the prefix-inventory
 * gate reads comments too, and a historical mention is indistinguishable to it
 * from a live one.) The feature has never run, so there is nothing to migrate --
 * the fix is to read the key that is written.
 *
