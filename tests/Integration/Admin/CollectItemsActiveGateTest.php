...
final class CollectItemsActiveGateTest extends WP_UnitTestCase {
	...
	public function test_passive_feature_is_not_collected_but_active_one_is(): void {
		...
		update_option( 'mhm_selected_features', array( 'bluetooth' ) ); // navigation is Passive
		...
		$stored_keys = array_column( $stored['mhm_rentiva_vehicle_card_fields'], 'key' );
		...
		update_option( 'mhm_selected_features', array( 'bluetooth', 'navigation' ) );
		$this->reset_map_cache();
