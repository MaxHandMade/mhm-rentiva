...
/**
 * Removing a custom detail must also clear its mhm_custom_field_meta entry, or an orphan
...
final class VehicleSettingsRemoveCustomFieldTest extends WP_Ajax_UnitTestCase {
	...
	public function test_removing_custom_detail_clears_its_field_meta(): void {
		...
		update_option( 'mhm_custom_details', array( 'custom_boot' => 'Boot Size' ) );
		...
		$this->assertArrayNotHasKey( 'custom_boot', (array) get_option( 'mhm_custom_details' ), 'the custom detail itself is removed' );

		$meta = (array) get_option( 'mhm_custom_field_meta' );
