...
class MigrationLaneIndependenceTest extends WP_UnitTestCase
{
    ...
    public function test_pollution_migration_runs_after_version_already_stamped(): void
    {
        ...
        // pollution in mhmrentiva_settings.
        update_option(self::VERSION_KEY, defined('MHMRENTIVA_VERSION') ? MHMRENTIVA_VERSION : 'current');
        ...
        $this->assertArrayNotHasKey('mhmrentiva_brand_name', $stored, 'Polluted brand name must be removed regardless of version stamp.');
        $this->assertArrayNotHasKey('mhmrentiva_currency', $stored, 'Polluted currency must be removed regardless of version stamp.');
    ...
    public function test_labels_migration_runs_after_version_already_stamped(): void
    {
        ...
        update_option('mhmrentiva_vehicle_features', array( 'air_conditioning' => 'Air Conditioning' ));
        delete_option(self::LABELS_FLAG);

        VehicleMeta::migrate_remove_auto_populated_labels();

        $this->assertFalse(get_option('mhmrentiva_vehicle_details'));
        $this->assertFalse(get_option('mhmrentiva_vehicle_features'));
    ...
    public function test_migrations_are_no_ops_after_flag_is_set(): void
    {
        ...
        update_option(self::SETTINGS_KEY, array( 'mhmrentiva_brand_name' => '1' ));

        SettingsCore::migrate_clean_test_pollution();

        $this->assertSame(
            array( 'mhmrentiva_brand_name' => '1' ),
