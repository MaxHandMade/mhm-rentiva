				...
				<div class="analytics-chart">
					<div class="chart-bars">
						...
						foreach ( $customer_segments as $segment => $count ) {
							...
							echo '<div class="chart-bar ' . esc_attr( $segment ) . '" style="height: ' . esc_attr( (string) $final_height ) . '%;">';
				...
				if ( ! empty( $real_customers ) ) {
					foreach ( $real_customers as $customer ) {
						...
						<tr>
							...
							<td><?php echo esc_html( number_format( (int) $customer->booking_count ) ); ?></td>
							<td><?php echo esc_html( number_format( (float) $customer->total_spent, 2, ',', '.' ) . \MHMRentiva\Admin\Reports\Reports::get_currency_symbol() ); ?></td>
