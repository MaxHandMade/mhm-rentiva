		...
		<div class="rv-search-main-row">
			...
			<div class="rv-search-field rv-search-datetime">
				...
				<div class="rv-datetime-wrapper">
					<input type="text" id="rv-pickup-date" name="pickup_date" class="rv-date-input" placeholder="<?php echo esc_attr__('Date', 'mhm-rentiva'); ?>" value="<?php echo esc_attr($form_data['pickup_date_formatted'] ?? ''); ?>" readonly />
					<select name="pickup_time" id="rv-pickup-time" class="rv-time-select">
						...
						for ($i = 0; $i < 24; $i++) {
							foreach (array('00', '30') as $min) {
								...
								echo '<option value="' . esc_attr($time) . '" ' . selected($selected_pickup, $time, false) . '>' . esc_html($time) . '</option>';
							}
						}
						?>
					</select>
			...
			<div class="rv-search-field rv-search-datetime">
				...
				<div class="rv-datetime-wrapper">
					<input type="text" id="rv-return-date" name="return_date" class="rv-date-input" placeholder="<?php echo esc_attr__('Date', 'mhm-rentiva'); ?>" value="<?php echo esc_attr($form_data['return_date_formatted'] ?? ''); ?>" readonly />
					<select id="rv-return-time" class="rv-time-select rv-time-select-disabled" disabled readonly>
						...
						for ($i = 0; $i < 24; $i++) {
							foreach (array('00', '30') as $min) {
								$time = sprintf('%02d:%s', $i, $min);
								echo '<option value="' . esc_attr($time) . '" ' . selected($selected_return, $time, false) . '>' . esc_html($time) . '</option>';
							}
						}
						?>
					</select>
					...
					<?php if (($atts['show_fuel_type'] ?? '1') === '1') : ?>
						...
						<div class="rv-search-field rv-search-fuel">
							...
							<select id="rv-fuel-type" name="fuel_type">
								...
								<?php foreach (($form_data['fuel_types'] ?? array()) as $key => $fuel_type) : ?>
					...
					<?php if (($atts['show_transmission'] ?? '1') === '1') : ?>
						...
						<div class="rv-search-field rv-search-transmission">
							...
							<select id="rv-transmission" name="transmission">
								...
								<?php foreach (($form_data['transmissions'] ?? array()) as $key => $transmission) : ?>
					...
					<?php if (($atts['show_seats'] ?? '1') === '1') : ?>
						...
						<div class="rv-search-field rv-search-seats">
							...
							<select id="rv-seats" name="seats">
								...
								<?php foreach (($form_data['seat_options'] ?? array()) as $key => $seats) : ?>
