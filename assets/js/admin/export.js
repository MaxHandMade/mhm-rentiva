...
(function ($) {
	...
	function initExportPage() {
		...
		$( document ).on(
			'click',
			'button[onclick="showCustomExportForm()"]',
			function () {
			...
			$.ajax(
				{
					...
					success: function (response) {
						if (response.success) {
							...
							$( 'button[onclick*="' + exportId + '"]' ).closest( 'tr' ).fadeOut(
								500,
								function () {
									$( this ).remove();
								}
