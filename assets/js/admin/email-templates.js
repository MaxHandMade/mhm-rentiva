	...
	function ($) {
		...
		var testEmail = {
			...
			bindEvents: function () {
				...
				$( document ).on(
					...
					function (e) {
						...
						if ( ! email) {
							const alertMsg = (mhmrentiva_email_templates_vars.strings && mhmrentiva_email_templates_vars.strings.enterEmail) || 'Please enter email address';
			...
			showTestForm: function (templateKey) {
				const sendTestTitle = (mhmrentiva_email_templates_vars.strings && mhmrentiva_email_templates_vars.strings.sendTestEmail) || 'Send Test Email';
				const emailLabel    = (mhmrentiva_email_templates_vars.strings && mhmrentiva_email_templates_vars.strings.emailAddress) || 'Email Address';
				const cancelText    = (mhmrentiva_email_templates_vars.strings && mhmrentiva_email_templates_vars.strings.cancel) || 'Cancel';
				...
				var form = $(
					...
					'<button type="submit" class="button button-primary">' + mhmrentiva_email_templates_vars.send_test + '</button>' +
			...
			sendTestEmail: function (templateKey, email, form) {
				...
				submitBtn.prop( 'disabled', true ).text( mhmrentiva_email_templates_vars.processing );
				...
				$.ajax(
					{
						...
						type: 'POST',
						data: {
							action: 'mhmrentiva_send_test_email',
							template_key: templateKey,
							test_email: email,
							nonce: mhmrentiva_email_templates_vars.nonce
						},
						success: function (response) {
							if (response.success) {
								showNotice( mhmrentiva_email_templates_vars.test_email_sent, 'success' );
								form.remove();
							} else {
								showNotice( response.data || mhmrentiva_email_templates_vars.test_email_failed, 'error' );
							}
						},
						error: function () {
							showNotice( mhmrentiva_email_templates_vars.error_occurred, 'error' );
						},
						complete: function () {
		...
		var templateSettings = {
			...
			editTemplate: function (templateKey) {
				const editTitle    = (mhmrentiva_email_templates_vars.strings && mhmrentiva_email_templates_vars.strings.editTemplate) || 'Edit Template';
				const subjectLabel = (mhmrentiva_email_templates_vars.strings && mhmrentiva_email_templates_vars.strings.subject) || 'Subject';
				const contentLabel = (mhmrentiva_email_templates_vars.strings && mhmrentiva_email_templates_vars.strings.content) || 'Content';
				const saveText     = (mhmrentiva_email_templates_vars.strings && mhmrentiva_email_templates_vars.strings.save) || 'Save';
				const cancelText   = (mhmrentiva_email_templates_vars.strings && mhmrentiva_email_templates_vars.strings.cancel) || 'Cancel';
				...
				$.ajax(
					{
						...
						type: 'POST',
						data: {
							action: 'mhmrentiva_get_template_data',
							template_key: templateKey,
							nonce: mhmrentiva_email_templates_vars.nonce
						},
						success: function (response) {
				...
				$.ajax(
					{
						...
						type: 'POST',
						data: {
							action: 'mhmrentiva_save_template',
							template_key: templateKey,
						...
						success: function (response) {
							if (response.success) {
								const successMsg = (mhmrentiva_email_templates_vars.strings && mhmrentiva_email_templates_vars.strings.templateSaved) || 'Template saved successfully!';
								showNotice( successMsg, 'success' );
								$( '.template-edit-modal' ).remove();
								location.reload();
							} else {
								showNotice( response.data || mhmrentiva_email_templates_vars.error_occurred, 'error' );
							}
						},
						error: function () {
							showNotice( mhmrentiva_email_templates_vars.error_occurred, 'error' );
				...
				$.ajax(
					{
						...
						type: 'POST',
						data: {
							action: 'mhmrentiva_reset_template',
							template_key: templateKey,
							nonce: mhmrentiva_email_templates_vars.nonce
						},
						success: function (response) {
							if (response.success) {
								const successMsg = (mhmrentiva_email_templates_vars.strings && mhmrentiva_email_templates_vars.strings.templateReset) || 'Template reset to default!';
								showNotice( successMsg, 'success' );
								location.reload();
							} else {
								showNotice( response.data || mhmrentiva_email_templates_vars.error_occurred, 'error' );
