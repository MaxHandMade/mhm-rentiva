	...
	function ($) {
		...
		var inlineEdit = {
			...
			saveEdit: function ($input) {
				...
				if (isNaN( newPrice ) || newPrice < 0) {
					const invalidMsg = (mhmrentiva_addon_list_vars.strings && mhmrentiva_addon_list_vars.strings.invalidPrice) || 'Invalid price value!';
				...
				$.ajax(
					{
						url: mhmrentiva_addon_list_vars.ajax_url,
						type: 'POST',
						data: {
							action: 'mhmrentiva_update_addon_price',
							addon_id: addonId,
							price: newPrice,
							nonce: mhmrentiva_addon_list_vars.nonce
						},
						success: function (response) {
							if (response.success) {
								...
								const unknownMsg = (mhmrentiva_addon_list_vars.strings && mhmrentiva_addon_list_vars.strings.unknownError) || 'Unknown error';
		...
		var bulkActions = {
				...
				$( '.bulk-actions .button' ).on(
					...
					function (e) {
						...
						if (selectedItems.length === 0) {
							showNotice( mhmrentiva_addon_list_vars.no_items_selected, 'warning' );
							return;
						}
			...
			updateBulkActions: function () {
				...
				if (selectedCount > 0) {
					$( '.bulk-actions .button' ).prop( 'disabled', false );
					$( '.bulk-actions .selected-count' ).text( selectedCount + ' ' + mhmrentiva_addon_list_vars.items_selected );
				} else {
					$( '.bulk-actions .button' ).prop( 'disabled', true );
			...
			enableAddons: function (selectedItems) {
				if ( ! confirm( mhmrentiva_addon_list_vars.confirm_enable )) {
					return;
				}
			...
			disableAddons: function (selectedItems) {
				if ( ! confirm( mhmrentiva_addon_list_vars.confirm_disable )) {
					return;
				}
			...
			deleteAddons: function (selectedItems) {
				if ( ! confirm( mhmrentiva_addon_list_vars.confirm_delete )) {
					return;
				}
			...
			performBulkAction: function (action, addonIds) {
				...
				$button.prop( 'disabled', true ).text( mhmrentiva_addon_list_vars.processing );
				...
				$.ajax(
					{
						...
						type: 'POST',
						data: {
							action: 'mhmrentiva_bulk_addon_action',
							bulk_action: action,
							addon_ids: addonIds,
							nonce: mhmrentiva_addon_list_vars.nonce
						},
						success: function (response) {
							if (response.success) {
								...
								showNotice( response.data || mhmrentiva_addon_list_vars.error_occurred, 'error' );
							}
						},
						error: function () {
							showNotice( mhmrentiva_addon_list_vars.error_occurred, 'error' );
						},
						complete: function () {
		...
		if (typeof mhmrentiva_addon_list_vars !== 'undefined' && mhmrentiva_addon_list_vars.auto_refresh) {
			setInterval(
				function () {
