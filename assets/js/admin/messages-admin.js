...
(function ($) {
    ...
    const MessagesAdmin = {
        ...
        initializeStatusSelects: function () {
            $('.message-status-select').each(function () {
                const $select = $(this);
        ...
        initializeBulkActions: function () {
            ...
            $('#doaction, #doaction2').on('change', function () {
                ...
                if (selectedAction === 'change_status') {
                    ...
                    if ($statusContainer.length === 0) {
                        ...
                        if (mhmMessagesAdmin && mhmMessagesAdmin.statuses) {
                            $.each(mhmMessagesAdmin.statuses, function (key, label) {
        ...
        performBulkAction: function (action, $items) {
            const itemIds = $items.map(function () {
                return $(this).val();
            }).get();
        ...
        handleReplySubmit: function (e) {
            ...
            $.ajax({
                ...
                error: function (xhr, status, error) {
                    console.error('[MessagesAdmin] AJAX request failed:', {
                        status: status,
                        error: error,
                        xhr: xhr,
        ...
        formatTimeAgo: function (dateString) {
            ...
            if (diffInSeconds < 60) return strings.justNow || 'Just now';
            if (diffInSeconds < 3600) return Math.floor(diffInSeconds / 60) + ' ' + (strings.minutesAgo || 'minutes ago');
            if (diffInSeconds < 86400) return Math.floor(diffInSeconds / 3600) + ' ' + (strings.hoursAgo || 'hours ago');
            if (diffInSeconds < 2592000) return Math.floor(diffInSeconds / 86400) + ' ' + (strings.daysAgo || 'days ago');
