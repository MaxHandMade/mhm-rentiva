...
(function ($) {
    ...
    $(function () {
        ...
        $form.on('change', 'select[name="m"], select[name="mhm_booking_status"], select[name="mhm_payment_status"], select[name="mhm_payment_gateway"]', function () {
            $form.trigger('submit');
        });

        // On Enter in Booking ID / License Plate → submit
        $form.on('keydown', 'input[name="mhm_booking_id"], input[name="mhm_license_plate"]', function (e) {
            if (e.key === 'Enter') {
                e.preventDefault();
                $form.trigger('submit');
            }
