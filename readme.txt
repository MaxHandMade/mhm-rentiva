...
* Added: seasonal pricing multipliers can now be configured. The screen that renders them previously had no way in, so despite being stored and applied they could not be adjusted at all; they are now on Settings → Vehicle Management, under "Vehicle Pricing Settings", together with a new master on/off switch. The switch defaults to off, so pricing is unchanged unless you turn it on.
...
* Fixed: a booking whose vehicle had no featured image showed a broken image on the My Account bookings list and booking detail screens, because the placeholder file those screens pointed at has never been part of this plugin. They now use the same neutral placeholder the vehicle grid and list already used.
...
* Fixed: adding a custom feature, equipment item or detail on the vehicle edit screen stopped with a JavaScript error, because the script called a function that does not exist anywhere in the plugin. Re-ordering existing items by dragging was never affected and is unchanged.
...
* Internal: the automated test suite is now 1,553 tests and passes with zero failures.
