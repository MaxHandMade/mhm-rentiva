...
* Fixed: a manually created booking whose deposit was taken outside WooCommerce could still be offered a WooCommerce payment link for its remaining balance, and using that link always failed -- the same guard that makes taking the deposit offline possible also refuses to build a WooCommerce order for money it already knows arrived elsewhere. The deposit screen now recognises this case and explains it in place of that button. "Process Remaining Amount," which records the balance as settled without going through WooCommerce, is unaffected and still works.
