...
* Fixed: a refund is now recorded once and announces itself once. A deposit booking's two paid orders each fired their own record-and-notify step for what was a single refund, so it used to be counted twice and e-mailed to the customer twice as soon as it succeeded.
...
* Fixed: the refund box on the booking screen appears again for offline bookings with a balance. It had never rendered its form on any site, because it required a paid amount read from a meta key nothing wrote.
