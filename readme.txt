...
= 6.1.4 =
* Fixed: a booking linked to a customer's account by ID rather than e-mail was invisible on the Customers screen, in that customer's detail panel, in the dashboard's money totals, and behind the "View Bookings" link. Everyone the customer list already counts as a customer is now counted everywhere else, too.
* Fixed: multisite activation stopped at the hundredth site on larger networks and skipped private sites; both now get their tables. A subsite reads its own language for its URL slugs instead of the network's.
* Fixed: on hosting with an external object cache (Redis/Memcached), saving a customer, booking or vehicle left its list showing stale data until the cache expired on its own.
