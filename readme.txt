...
* Fixed: a vehicle with no featured image showed a broken image on the My Account bookings list and booking detail screens, and on the alternative-vehicle cards the booking form offers when the dates you picked are taken. The placeholder file all of them pointed at has never been part of this plugin, and the booking form additionally built the address for it in a way that only worked on a default WordPress layout. All of them now use the same neutral placeholder the vehicle grid and list already used.
