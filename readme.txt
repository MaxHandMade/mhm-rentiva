...
* ✨ **Feat:** Vendor names in the Active Vendors table open a read-only detail panel (contact, masked IBAN, status, score, and the vendor's vehicles incl. archived) via a new `GET /vendors/vendors/{id}` endpoint. The misleading row pointer is scoped to the clickable Applications table only.
