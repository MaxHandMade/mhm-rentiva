...
/**
 * Mesajlaşma sistemi için cache yönetimi
 */
final class MessageCache
{
     ...
     * Cache key oluştur
     */
    private static function get_cache_key(string $key, array $params = []): string
    {
        if (!empty($params)) {
     ...
     * Cache'e veri kaydet
     */
    public static function set(string $key, $data, array $params = [], int $expiry = null): bool
    {
        $cache_key = self::get_cache_key($key, $params);
     ...
     * Cache'den veri sil
     */
    public static function delete(string $key, array $params = []): bool
    {
        $cache_key = self::get_cache_key($key, $params);
    ...
    /**
     * Dashboard widget verilerini cache'le
     */
    public static function get_dashboard_data(): array
    {
        ...
        if ($data === false) {
            ...
            $max_messages = 5; // Varsayılan değer, ayarlardan alınabilir
            ...
            self::set($cache_key, $data, [], 1800); // 30 dakika cache
        }
    ...
    /**
     * Mesaj oluşturulduğunda cache'i temizle
    ...
    {
        // Önce tüm cache grubunu temizle (en garantili yöntem)
        self::flush();
        
        // Genel cache'leri tekrar temizle (kesinlik için)
        self::delete('message_counts');
        self::delete('dashboard_data');
        self::delete('message_stats');
        
        // Müşteri özel cache'leri temizle
        if ($email) {
            self::delete('customer_messages', ['email' => $email]);
            self::delete('message_counts', ['email' => $email]);
        }
        
        // Thread cache'ini temizle
        if ($message_id) {
            ...
            if ($message_id_int) {
                ...
                if ($thread_id) {
                    self::delete('thread_messages', ['thread_id' => $thread_id]);
                }
            }
        }
        
        // WordPress object cache'ini de temizle (bazı durumlarda wp_cache_flush_group çalışmayabilir)
        if (function_exists('wp_cache_flush')) {
            // Bu çok agresif ama cache sorununu çözer
            // Alternatif: Sadece message cache'lerini temizlemek için wp_cache_delete_group kullanılabilir
            // Ancak wp_cache_delete_group WordPress 6.1+ için mevcut
        }
    }

    /**
     * Mesaj durumu değiştiğinde cache'i temizle
     */
