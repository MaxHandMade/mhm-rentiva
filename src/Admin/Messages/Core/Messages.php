...
final class Messages
{
    public static function register(): void
    {
        ...
        // AJAX handlers
        add_action('wp_ajax_mhm_messages_bulk_action', [self::class, 'handle_bulk_actions']);
    ...
    public static function render_dashboard_widget(): void
    {
        ...
        <div class="mhm-messages-dashboard">
            <div class="messages-count">
                ...
                <span class="count-label"><?php _e('Pending Message', 'mhm-rentiva'); ?></span>
            </div>

            <?php if (!empty($recent_messages)): ?>
                <div class="recent-messages">
                    <h4><?php _e('Recent Messages', 'mhm-rentiva'); ?></h4>
                    <ul>
                        <?php foreach ($recent_messages as $message): ?>
                            <li>
                                <a href="<?php echo esc_url(MessageUrlHelper::get_message_view_url($message->ID)); ?>">
                                    ...
                                    <br>
                                    <small class="message-date"><?php echo esc_html(human_time_diff(strtotime($message->post_date), current_time('timestamp'))); ?> <?php _e('ago', 'mhm-rentiva'); ?></small>
                                </a>
                            </li>
                        <?php endforeach; ?>
                    </ul>
            ...
            <div class="messages-actions">
                <a href="<?php echo esc_url(MessageUrlHelper::get_messages_list_url()); ?>" class="button button-primary">
                    <?php _e('View All Messages', 'mhm-rentiva'); ?>
                </a>
            </div>
        </div>
        <?php
    ...
    public static function render_messages_page(): void
    {
        ...
        echo '<h1 class="wp-heading-inline">' . esc_html__('Messages', 'mhm-rentiva') . '</h1>';
    ...
    private static function render_message_thread(int $message_id): void
    {
        ...
        if (empty($thread_messages)) {
            ...
            if ($main_message) {
                ...
                if (empty($main_message_obj->post_content)) {
                    global $wpdb;
                    $main_message_obj->post_content = $wpdb->get_var($wpdb->prepare(
                        "SELECT post_content FROM {$wpdb->posts} WHERE ID = %d",
                        $message_id
                    ));
                }
                $main_message_obj->meta = $meta;
                $thread_messages = [$main_message_obj];
            }
        } else {
            ...
            foreach ($thread_messages as &$msg) {
                if ($msg && isset($msg->ID)) {
                    ...
                    if (empty($msg->post_content)) {
                        global $wpdb;
                        $msg->post_content = $wpdb->get_var($wpdb->prepare(
                            "SELECT post_content FROM {$wpdb->posts} WHERE ID = %d",
                            $msg->ID
                        ));
                    }
                ...
                <div class="message-item <?php echo esc_attr($message_type); ?>">
                    <div class="message-header">
                        <div class="message-author">
                            <?php if ($is_customer_message): ?>
                                <strong><?php echo esc_html($thread_message->meta['customer_name'] ?? __('Customer', 'mhm-rentiva')); ?></strong>
                                <span class="message-type customer"><?php _e('Customer', 'mhm-rentiva'); ?></span>
                                <?php if ($show_new_badge): ?>
                                    <span class="status-badge-new" style="background: #ff9800; color: white; padding: 2px 8px; border-radius: 3px; font-size: 11px; font-weight: 600; margin-left: 8px; text-transform: uppercase;"><?php _e('New', 'mhm-rentiva'); ?></span>
                                <?php endif; ?>
                            <?php else: ?>
                                <strong><?php echo esc_html(get_the_author_meta('display_name', $thread_message->post_author)); ?></strong>
                                <span class="message-type admin"><?php _e('Administrator', 'mhm-rentiva'); ?></span>
                            <?php endif; ?>
                        </div>
                        <div class="message-date">
                            <?php echo esc_html(human_time_diff(strtotime($thread_message->post_date), current_time('timestamp'))); ?> <?php _e('ago', 'mhm-rentiva'); ?>
                        </div>
                    </div>
                    ...
                    <?php if (!empty($thread_message->meta['attachments'])): ?>
                        <div class="message-attachments">
                            <strong><?php _e('Attachments:', 'mhm-rentiva'); ?></strong>
    ...
    private static function render_reply_form(int $message_id): void
    {
        ...
        <div class="message-reply-form">
            ...
            <h2><?php _e('Send Reply', 'mhm-rentiva'); ?></h2>

            <div class="reply-info">
                <p><strong><?php _e('Recipient:', 'mhm-rentiva'); ?></strong> <?php echo esc_html($meta['customer_name']); ?> (<?php echo esc_html($meta['customer_email']); ?>)</p>
                <p><strong><?php _e('Subject:', 'mhm-rentiva'); ?></strong> <?php echo esc_html($message->post_title); ?></p>
            </div>
            ...
            <?php if (count($thread_messages) > 0): ?>
                <div class="message-thread-preview" style="margin: 20px 0; padding: 15px; background: #f9f9f9; border: 1px solid #ddd; border-radius: 4px;">
                    <h3 style="margin-top: 0; margin-bottom: 15px; font-size: 14px; font-weight: 600;">
                        <?php _e('Previous Messages', 'mhm-rentiva'); ?>
                    </h3>
                                ...
                                <div class="message-header" style="display: flex; justify-content: space-between; margin-bottom: 8px; font-size: 12px;">
                                    <div class="message-author">
                                        <?php if ($thread_message->meta['message_type'] === 'customer_to_admin'): ?>
                                            ...
                                            <span class="message-type customer" style="color: #2271b1; margin-left: 8px;"><?php _e('Customer', 'mhm-rentiva'); ?></span>
                                        <?php else: ?>
                                            <strong><?php echo esc_html(get_the_author_meta('display_name', $thread_message->post_author)); ?></strong>
                                            <span class="message-type admin" style="color: #00a32a; margin-left: 8px;"><?php _e('Administrator', 'mhm-rentiva'); ?></span>
                                        <?php endif; ?>
                                    </div>
            ...
            <form id="message-reply-form" method="post" action="">
                ...
                <div class="form-field">
                    <label for="reply_subject"><?php _e('Subject:', 'mhm-rentiva'); ?></label>
                    <input type="text" id="reply_subject" name="subject" value="Re: <?php echo esc_attr($message->post_title); ?>" class="regular-text" required>
                </div>

                <div class="form-field">
                    <label for="reply_message"><?php _e('Your Reply:', 'mhm-rentiva'); ?></label>
                    <?php
                    wp_editor('', 'reply_message', [
                ...
                <div class="form-field">
                    <label>
                        ...
                        <?php _e('Close conversation after this reply', 'mhm-rentiva'); ?>
                    </label>
                </div>

                <div class="form-actions">
                    <button type="submit" class="button button-primary"><?php _e('Send Reply', 'mhm-rentiva'); ?></button>
                    <a href="<?php echo esc_url(MessageUrlHelper::get_message_view_url($message_id)); ?>" class="button"><?php _e('Cancel', 'mhm-rentiva'); ?></a>
                </div>
            </form>
        </div>
        <?php
    ...
    {
        check_ajax_referer('mhm_messages_bulk_action', 'nonce');
        ...
        $message_ids = isset($_POST['message_ids']) ? array_map('absint', $_POST['message_ids']) : [];
    ...
    {
        check_ajax_referer('mhm_message_reply', 'nonce');
    ...
    {
        check_ajax_referer('mhm_message_status_update', 'nonce');
    ...
    {
        check_ajax_referer('mhm_message_reopen', 'nonce');
        ...
        if (Message::update_message_status($message_id, $new_status)) {
            ...
            wp_send_json_success([
                'message' => __('Message reopened successfully.', 'mhm-rentiva'),
                'status' => $new_status,
                'status_label' => MessagesSettings::get_statuses()[$new_status]
            ]);
        } else {
            wp_send_json_error(__('Message could not be reopened.', 'mhm-rentiva'));
        }
    }

    /**
     * Old enqueue_scripts function - removed (merged with enqueue_message_scripts)
     */
    ...
    {
        if (!current_user_can('manage_options')) {
            ErrorHandler::permission_error(__('You do not have permission to access this page.', 'mhm-rentiva'));
        }
        ...
        // Handle bulk actions BEFORE any output (to avoid headers already sent error)
        if (isset($_POST['action']) || isset($_POST['action2'])) {
            $list_table_temp = new MessageListTable();
    ...
    public static function handle_new_message_form(): void
    {
        ...
        wp_safe_redirect(MessageUrlHelper::get_messages_list_url_with_filters(['message' => 'sent']));
        exit;
    }
    ...
    private static function render_edit_form(int $message_id): void
    {
        ...
        <div class="message-edit-form">
            ...
            <h2><?php _e('Edit Message', 'mhm-rentiva'); ?></h2>

            <div class="message-info" style="margin: 20px 0; padding: 15px; background: #f9f9f9; border: 1px solid #ddd; border-radius: 4px;">
                <p><strong><?php _e('Subject:', 'mhm-rentiva'); ?></strong> <?php echo esc_html($message->post_title); ?></p>
                <p><strong><?php _e('Customer:', 'mhm-rentiva'); ?></strong> <?php echo esc_html($meta['customer_name']); ?> (<?php echo esc_html($meta['customer_email']); ?>)</p>
                <p><strong><?php _e('Date:', 'mhm-rentiva'); ?></strong> <?php echo esc_html(date_i18n(get_option('date_format') . ' ' . get_option('time_format'), strtotime($message->post_date))); ?></p>
                <p><strong><?php _e('Message Content:', 'mhm-rentiva'); ?></strong></p>
            ...
            <form method="post" action="">
                ...
                <table class="form-table">
                    <tr>
                        <th scope="row">
                            <label for="category"><?php _e('Category', 'mhm-rentiva'); ?></label>
                        </th>
                        <td>
                            ...
                            <p class="description"><?php _e('Select the message category.', 'mhm-rentiva'); ?></p>
                        </td>
                    </tr>

                    <tr>
                        <th scope="row">
                            <label for="status"><?php _e('Status', 'mhm-rentiva'); ?></label>
                        </th>
                        <td>
                            ...
                            <p class="description"><?php _e('Select the message status.', 'mhm-rentiva'); ?></p>
                        </td>
                    </tr>

                    <tr>
                        <th scope="row">
                            <label for="booking_id"><?php _e('Booking Association', 'mhm-rentiva'); ?></label>
                        </th>
                        <td>
                            ...
                            <p class="description"><?php _e('Associate this message with a booking (optional).', 'mhm-rentiva'); ?></p>
                        </td>
                    </tr>
                </table>

                <p class="submit">
                    <button type="submit" class="button button-primary"><?php _e('Update Message', 'mhm-rentiva'); ?></button>
                    <a href="<?php echo esc_url(MessageUrlHelper::get_message_view_url($message_id)); ?>" class="button"><?php _e('Cancel', 'mhm-rentiva'); ?></a>
                </p>
            </form>
        </div>
    ...
    public static function add_message_stats_cards(): void
    {
        ...
        <div class="wrap">
            ...
            <a href="<?php echo esc_url(MessageUrlHelper::get_new_message_url()); ?>" class="page-title-action">
                <?php echo esc_html__('New Message', 'mhm-rentiva'); ?>
            </a>
        ...
        <div class="mhm-stats-cards">
            <div class="stats-grid">
                <!-- Toplam Mesajlar -->
                <div class="stat-card stat-card-total-messages">
                    <div class="stat-icon">
                        <span class="dashicons dashicons-email-alt"></span>
                    </div>
                    <div class="stat-content">
                        <div class="stat-number"><?php echo esc_html($stats['total_messages']); ?></div>
                        <div class="stat-label"><?php esc_html_e('Total Messages', 'mhm-rentiva'); ?></div>
                        <div class="stat-trend">
                            <span class="trend-text"><?php esc_html_e('All messages', 'mhm-rentiva'); ?></span>
                        </div>
                    </div>
                </div>

                <!-- Pending Messages -->
                <div class="stat-card stat-card-pending-messages">
                    <div class="stat-icon">
                        <span class="dashicons dashicons-clock"></span>
                    </div>
                    <div class="stat-content">
                        <div class="stat-number"><?php echo esc_html($stats['pending_messages']); ?></div>
                        <div class="stat-label"><?php esc_html_e('Pending Messages', 'mhm-rentiva'); ?></div>
                        <div class="stat-trend">
                            <span class="trend-text trend-up"><?php echo esc_html($stats['pending_percentage']); ?>% <?php esc_html_e('pending', 'mhm-rentiva'); ?></span>
                        </div>
                    </div>
                </div>

                <!-- Answered Messages -->
                <div class="stat-card stat-card-answered-messages">
                    <div class="stat-icon">
                        <span class="dashicons dashicons-yes-alt"></span>
                    </div>
                    <div class="stat-content">
                        <div class="stat-number"><?php echo esc_html($stats['answered_messages']); ?></div>
                        <div class="stat-label"><?php esc_html_e('Answered', 'mhm-rentiva'); ?></div>
                        <div class="stat-trend">
                            <span class="trend-text trend-up"><?php echo esc_html($stats['answered_percentage']); ?>% <?php esc_html_e('answered', 'mhm-rentiva'); ?></span>
                        </div>
                    </div>
                </div>

                <!-- Average Response Time -->
                <div class="stat-card stat-card-avg-response-time">
                    <div class="stat-icon">
                        <span class="dashicons dashicons-performance"></span>
                    </div>
                    <div class="stat-content">
                        <div class="stat-number"><?php echo esc_html($stats['avg_response_time']); ?></div>
                        <div class="stat-label"><?php esc_html_e('Avg. Response Time', 'mhm-rentiva'); ?></div>
                        <div class="stat-trend">
                            <span class="trend-text"><?php esc_html_e('In hours', 'mhm-rentiva'); ?></span>
                        </div>
                    </div>
                </div>
            </div>
