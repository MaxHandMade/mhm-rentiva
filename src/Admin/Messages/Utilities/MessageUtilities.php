...
final class MessageUtilities
{
	...
	public static function get_message_counts(?string $email = null): array
	{
		...
		if (! empty($params)) {
			// phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared -- Query string built dynamically but variables prepared safely.
			$result = $wpdb->get_row($wpdb->prepare($query, ...$params), ARRAY_A);
		} else {
			// phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared -- Constants only.
