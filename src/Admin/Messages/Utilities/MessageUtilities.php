...
final class MessageUtilities
{
    ...
    public static function get_message_meta(int $message_id): array
    {
        return [
            ...
            'attachments' => get_post_meta($message_id, '_mhm_attachments', true) ?: [],
        ];
    }
    ...
    public static function can_send_message(string $email): array
    {
        ...
        if (!class_exists('\MHMRentiva\Admin\Licensing\Mode') || !\MHMRentiva\Admin\Licensing\Mode::isPro()) {
            ...
            $result['limits'] = [
                'daily' => ['used' => $today_count, 'limit' => $daily_limit],
                'monthly' => ['used' => $counts['total'], 'limit' => $monthly_limit]
            ];
        }
     ...
     * Load template file
     */
    public static function load_template(string $template_name, array $variables = []): string
    {
     ...
     * Write debug log
     */
    public static function debug_log(string $message, array $data = []): void
    {
