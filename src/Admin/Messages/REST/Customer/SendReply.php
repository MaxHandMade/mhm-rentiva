...
// phpcs:disable WordPress.DB.SlowDBQuery.slow_db_query_meta_query,WordPress.DB.SlowDBQuery.slow_db_query_meta_key,WordPress.DB.SlowDBQuery.slow_db_query_meta_value,WordPress.DB.SlowDBQuery.slow_db_query_tax_query,WordPress.DB.DirectDatabaseQuery.DirectQuery,WordPress.DB.DirectDatabaseQuery.NoCaching -- Customer reply endpoint intentionally performs bounded message/thread validation queries.
...
final class SendReply {
	...
	public static function handle( WP_REST_Request $request ): WP_REST_Response {
		...
		if ( empty( $thread_messages ) ) {
			return new WP_REST_Response( array( 'error' => __( 'Thread not found or empty', 'mhm-rentiva' ) ), 404 );
		}

		$last_message = end( $thread_messages );

		if ( ! $last_message || ! isset( $last_message->ID ) ) {
			return new WP_REST_Response( array( 'error' => __( 'Invalid thread data', 'mhm-rentiva' ) ), 400 );
		}
		...
		if ( $last_meta['status'] === 'closed' ) {
			return new WP_REST_Response( array( 'error' => __( 'This conversation is closed', 'mhm-rentiva' ) ), 400 );
		}
