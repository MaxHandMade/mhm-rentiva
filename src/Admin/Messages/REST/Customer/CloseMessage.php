...
final class CloseMessage
{
    ...
    public static function handle(WP_REST_Request $request): WP_REST_Response
    {
        ...
        if (!$thread_id) {
            return new WP_REST_Response(['error' => __('Thread ID is required.', 'mhm-rentiva')], 400);
        }
        ...
        $main_message_id = $wpdb->get_var($wpdb->prepare(
            "SELECT p.ID 
             ...
             LIMIT 1",
            $thread_id, $customer_email
        ));

        if (!$main_message_id) {
            return new WP_REST_Response(['error' => __('Message not found or access denied.', 'mhm-rentiva')], 404);
        }
        ...
        if (!$updated) {
            return new WP_REST_Response(['error' => __('Failed to close message.', 'mhm-rentiva')], 400);
        }
