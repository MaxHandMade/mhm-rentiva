 ...
 */
// phpcs:disable WordPress.DB.DirectDatabaseQuery.DirectQuery,WordPress.DB.DirectDatabaseQuery.NoCaching -- This file is the schema migrator: CREATE/ALTER/DROP and one-shot data backfills. WordPress has no core API for DDL. Every statement below is version-gated and runs once per install; each one changes the rows it just read, so there is nothing to cache. Tables touched are this plugin's own plus wp_posts/wp_postmeta/wp_options during the 6.0.0 rename. SchemaChange is not suppressed anywhere in this file: every DDL statement below reports it. Scope is bounded by the version gate, not by user input.
final class DatabaseMigrator {
	 ...
	 *
	 * The retry that keeps this version-gated migration from stranding a
	 * half-cleaned install has always run on every `admin_init` (Plugin.php),
	 * a hook that fires for an
	...
	private static function migrate_vehicle_lifecycle_status(): void
	{
		...
		// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery,WordPress.DB.DirectDatabaseQuery.NoCaching -- A one-shot backfill during a version-gated migration: a LEFT JOIN ... IS NULL anti-join has no WP_Query form (meta_query cannot express "row absent" without loading every vehicle). The migration runs once and then changes the rows it just read, so there is nothing to cache.
	...
	{
		global $wpdb;
		// phpcs:disable WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- Dropping dead orchestration tables IS this method. There is no core API for DROP TABLE. One-shot DDL has nothing to cache.
	 ...
	 * pre-6.0.0 spelling -- the add-on's transfer meta box's luggage and
	 * pricing reads are the clearest -- and they are NOT redundant during the
	 * transition. The
