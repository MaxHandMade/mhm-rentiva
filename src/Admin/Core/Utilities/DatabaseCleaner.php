...
final class DatabaseCleaner
{
    ...
    public static function cleanup_invalid_meta_keys(bool $dry_run = true): array
    {
        ...
        if ($dry_run) {
            return [
                'dry_run' => true,
                'would_delete' => array_sum(array_column($invalid_data['keys'] ?? [], 'count')),
    ...
    public static function cleanup_expired_transients(bool $dry_run = true): array
    {
        ...
        if ($count > 0) {
            ...
            // Note: Cannot use prepare() for table names, only for values
            $deleted = $wpdb->query("
                DELETE o1, o2
                FROM {$wpdb->options} o1
                INNER JOIN {$wpdb->options} o2 
     ...
     */
    public static function full_cleanup(bool $dry_run = true, array $options = []): array
    {
        $default_options = [
        ...
        if ($options['orphaned_usermeta']) {
            $orphaned_usermeta = self::find_orphaned_usermeta();
            $results['operations']['orphaned_usermeta'] = [
                'checked' => true,
...
<?php
            ...
            foreach ($files as $file) {
                ...
                if (!$exists_in_db) {
                    $backups[] = [
                        ...
                        'file_size_mb' => round(filesize($file) / 1024 / 1024, 2),
    ...
    private static function secure_backup_directory(string $directory): void
    {
        ...
        if (function_exists('chmod')) {
            ...
            @chmod($htaccess_file, 0644); // Owner: read/write, Group/Others: read
            @chmod($index_file, 0644); // Owner: read/write, Group/Others: read
        }
    }
