 ...
 * 
 * WordPress MetaBox sınıfları için merkezi base class.
...
abstract class AbstractMetaBox
{
    ...
    public static function render_meta_box(\WP_Post $post, array $args = []): void
    {
        ...
        if (isset($field_config['template']) && method_exists(static::class, $field_config['template'])) {
            call_user_func([static::class, $field_config['template']], $post, $field_config);
        } else {
    ...
    protected static function render_default_template(\WP_Post $post, array $field_config, string $meta_box_id): void
    {
        ...
        foreach ($field_config['fields'] ?? [] as $field_key => $field) {
    ...
    protected static function render_radio_field(string $field_key, $value, array $field): void
    {
        ...
        foreach ($options as $option_value => $option_label) {
            ...
            echo '<label>';
            echo '<input type="radio" name="' . esc_attr($field_key) . '" value="' . esc_attr($option_value) . '" ' . $checked . '> ';
    ...
    public static function save_meta(int $post_id, \WP_Post $post): void
    {
        ...
        foreach ($fields as $meta_box_id => $field_config) {
            ...
            if (isset($field_config['save_handler']) && method_exists(static::class, $field_config['save_handler'])) {
                call_user_func([static::class, $field_config['save_handler']], $post_id, $field_config);
            } else {
                // Default save
    ...
    protected static function save_field(int $post_id, string $field_key, array $field): void
    {
        ...
        if ($sanitize_callback && is_callable($sanitize_callback)) {
            // sanitize_callback'e null geçmemek için kontrol
    ...
    /**
     * Helper: Meta değerleri toplu al
     */
    ...
    /**
     * Helper: Meta değerleri toplu kaydet
     */
    ...
    /**
     * Helper: Meta değerleri toplu sil
     */
    ...
    /**
     * Helper: Custom field render (override edilebilir)
     */
    ...
    /**
     * Helper: Custom field save (override edilebilir)
     */
