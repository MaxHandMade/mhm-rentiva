...
final class SettingsSanitizer
{
    ...
    public static function sanitize(mixed $input): array
    {
        ...
        $current_values = (array) \get_option('mhm_rentiva_settings', []);
    ...
    private static function process_general_tab(array $input, array $defaults): array
    {
        ...
        $fields = ['mhm_rentiva_currency_position', 'mhm_rentiva_date_format', 'mhm_rentiva_time_format', 'mhm_rentiva_dark_mode'];
    ...
    private static function apply_global_constraints(array $input, array $out, string $tab): array
    {
        if (isset($input['mhm_rentiva_booking_payment_deadline_minutes']) && 'booking' === $tab) {
            $out['mhm_rentiva_booking_payment_deadline_minutes'] = clamp((int) $input['mhm_rentiva_booking_payment_deadline_minutes'], 0, 1440);
        }

        if (isset($input['mhm_rentiva_booking_payment_gateway_timeout_minutes'])) {
            $out['mhm_rentiva_booking_payment_gateway_timeout_minutes'] = clamp((int) $input['mhm_rentiva_booking_payment_gateway_timeout_minutes'], 0, 60);
        }
    ...
    {
        return isset($input[$key]) ? clamp((int) $input[$key], $min, $max) : $default;
    }
    ...
    private static function sanitize_system_settings(array $input, array $defaults): array
    {
        return [
            ...
            // Maintenance
            'mhm_rentiva_log_level'                  => self::validate_enum($input['mhm_rentiva_log_level'] ?? '', ['error', 'warning', 'info', 'debug'], 'error'),
    ...
    private static function sanitize_vehicle_management_settings(array $input, array $defaults): array
    {
        $out = [
            ...
            'mhm_rentiva_vehicle_tax_rate'             => clamp(floatval($input['mhm_rentiva_vehicle_tax_rate'] ?? 0), 0, 100),
            'mhm_rentiva_vehicle_cards_per_page'      => self::get_int($input, 'mhm_rentiva_vehicle_cards_per_page', 12, 1, 100),
            'mhm_rentiva_vehicle_default_sort'         => self::validate_enum($input['mhm_rentiva_vehicle_default_sort'] ?? '', ['price_asc', 'price_desc', 'name_asc', 'name_desc', 'year_desc', 'year_asc'], 'price_asc'),
    ...
    {
        $current_settings = (array) \get_option('mhm_rentiva_settings', []);
        $current_pricing  = $current_settings['vehicle_pricing'] ?? ($defaults['vehicle_pricing'] ?? []);
        ...
        if (isset($input['vehicle_pricing']) && \is_array($input['vehicle_pricing'])) {
            ...
            if (isset($in['deposit_settings']) && \is_array($in['deposit_settings'])) {
                ...
                $current_pricing['deposit_settings'] = [
                    ...
                    'deposit_payment_methods' => \is_array($dep['deposit_payment_methods'] ?? null)
                        ? array_map([self::class, 'safe_text'], $dep['deposit_payment_methods'])
            ...
            if (isset($in['discount_options']) && \is_array($in['discount_options'])) {
                foreach ($in['discount_options'] as $key => $discount) {
                    $current_pricing['discount_options'][$key] = [
                        'enabled'          => (bool) ($discount['enabled'] ?? false),
                        'min_days'         => \absint($discount['min_days'] ?? 0),
                        'advance_days'     => \absint($discount['advance_days'] ?? 0),
                        'discount_percent' => clamp(\absint($discount['discount_percent'] ?? 0), 0, 100),
                    ];
                }
            }
        }

        return ['vehicle_pricing' => $current_pricing];
    }
    ...
    private static function sanitize_frontend_settings(array $input, array $defaults): array
    {
        ...
        $slugs = ['mhm_rentiva_endpoint_bookings', 'mhm_rentiva_endpoint_favorites', 'mhm_rentiva_endpoint_payment_history', 'mhm_rentiva_endpoint_messages', 'mhm_rentiva_endpoint_edit_account'];
        foreach ($slugs as $s) {
            $out[$s] = \sanitize_title($input[$s] ?? ($defaults[$s] ?? ''));
        }

        $urls = ['mhm_rentiva_booking_url', 'mhm_rentiva_login_url', 'mhm_rentiva_register_url', 'mhm_rentiva_vehicles_list_url', 'mhm_rentiva_search_url', 'mhm_rentiva_contact_url'];
        foreach ($urls as $u) {
    ...
    private static function sanitize_comments_settings(array $input, array $current): array
    {
        ...
        return [
            'comments_approval' => [
                ...
                'require_login'  => (bool) ($in['approval']['require_login'] ?? false),
                'admin_notification' => (bool) ($in['approval']['admin_notification'] ?? false),
            ],
            'comments_limits' => [
                'comments_per_page' => clamp((int) ($in['limits']['comments_per_page'] ?? 10), 1, 100),
                'comment_length_max' => clamp((int) ($in['limits']['comment_length_max'] ?? 1000), 10, 5000),
            ],
            'comments_spam_protection' => [
                'enabled' => (bool) ($in['spam_protection']['enabled'] ?? false),
                'spam_words' => \is_array($in['spam_protection']['spam_words'] ?? null)
                    ? array_map([self::class, 'safe_text'], $in['spam_protection']['spam_words'])
                    : [],
            ],
        ];
    }
    ...
    private static function sanitize_comparison_settings(array $input, array $current): array
    {
        ...
        return ['comparison_fields' => $sanitized];
    }
    ...
    private static function sanitize_addon_settings(array $input, array $defaults): array
    {
        return [
            ...
            'mhm_rentiva_addon_display_order'           => self::validate_enum($input['mhm_rentiva_addon_display_order'] ?? '', ['menu_order', 'title', 'price_asc', 'price_desc', 'date_created'], 'menu_order'),
        ];
    }
    ...
    {
        return [
            'mhm_transfer_deposit_type' => self::validate_enum($input['mhm_transfer_deposit_type'] ?? '', ['full_payment', 'percentage'], 'full_payment'),
...
/**
 * Polyfill for clamp if using PHP < 8.0 logic (though 8.2 is target)
 */
