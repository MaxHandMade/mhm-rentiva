...
final class ShortcodePages
{
    ...
    public function enqueue_assets(): void
    {
        ...
        wp_localize_script(self::SCRIPT_HANDLE, 'mhmShortcodePages', [
    ...
    public function register_ajax_handlers(): void
    {
        ...
        foreach ($handlers as $action => $method) {
            if (method_exists($this->ajax, $method)) {
                add_action("wp_ajax_{$action}", [$this->ajax, $method]);
            }
        }
    }
    ...
    public function render_page(): void
    {
        ...
        try {
            $this->render_view('admin/shortcode-pages', [
     ...
     * @return void
     */
    private function render_view(string $template, array $args = []): void
    {
        ...
        if (false === $templates_dir) {
            throw new RuntimeException(__('Templates directory not found.', 'mhm-rentiva'));
        }
        ...
        if (false === $real_file || !str_starts_with($real_file, $templates_dir)) {
            throw new InvalidArgumentException(
                sprintf(__('Illegal template access: %s', 'mhm-rentiva'), esc_html($template))
            );
        }
