...
// phpcs:disable WordPress.DB.SlowDBQuery.slow_db_query_meta_query,WordPress.DB.SlowDBQuery.slow_db_query_meta_key,WordPress.DB.SlowDBQuery.slow_db_query_meta_value,WordPress.DB.SlowDBQuery.slow_db_query_tax_query,WordPress.DB.DirectDatabaseQuery.DirectQuery,WordPress.DB.DirectDatabaseQuery.NoCaching -- Settings test routines intentionally run direct consistency checks.
...
final class SettingsTester {
	...
	private static function test_frontend_settings(): array {
		...
		foreach ( $url_fields as $field ) {
			...
			if ( ! empty( $value ) ) {
				...
				$is_path     = preg_match( '#^/.*#', $value );
				$is_relative = preg_match( '#^[a-zA-Z0-9\-_]+$#', $value );

				if ( ! $is_url && ! $is_path && ! $is_relative ) {
	...
	private static function test_transfer_settings(): array {
		...
		$locations_table = $wpdb->prefix . 'rentiva_transfer_locations';
		...
		// 5. Data Validation (Check if engine is ready)
		if ( $results['locations_table_exists'] ) {
			$results['has_locations'] = true;
		} else {
