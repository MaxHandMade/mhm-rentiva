...
final class DatabaseCleanupPage
{
    public static function register(): void
    {
        ...
        add_action('wp_ajax_mhm_analyze_database', [self::class, 'ajax_analyze']);
        add_action('wp_ajax_mhm_cleanup_orphaned', [self::class, 'ajax_cleanup_orphaned']);
        add_action('wp_ajax_mhm_cleanup_transients', [self::class, 'ajax_cleanup_transients']);
        add_action('wp_ajax_mhm_optimize_autoload', [self::class, 'ajax_optimize_autoload']);
        add_action('wp_ajax_mhm_optimize_tables', [self::class, 'ajax_optimize_tables']);
        add_action('wp_ajax_mhm_cleanup_invalid_meta', [self::class, 'ajax_cleanup_invalid_meta']);
        add_action('wp_ajax_mhm_list_backups', [self::class, 'ajax_list_backups']);
        add_action('wp_ajax_mhm_download_backup', [self::class, 'ajax_download_backup']);
        add_action('wp_ajax_mhm_restore_backup', [self::class, 'ajax_restore_backup']);
        add_action('wp_ajax_mhm_delete_backup', [self::class, 'ajax_delete_backup']);
        add_action('wp_ajax_mhm_create_full_backup', [self::class, 'ajax_create_full_backup']);
        add_action('wp_ajax_mhm_list_full_backups', [self::class, 'ajax_list_full_backups']);
        add_action('wp_ajax_mhm_download_full_backup', [self::class, 'ajax_download_full_backup']);
    ...
    public static function enqueue_assets(string $hook): void
    {
        ...
        wp_enqueue_style(
            ...
            esc_url(plugin_dir_url(dirname(__FILE__, 4)) . 'assets/css/admin/database-cleanup.css'),
            [],
            '1.0.0'
        );
        ...
        wp_enqueue_script(
            'mhm-database-cleanup',
            esc_url(plugin_dir_url(dirname(__FILE__, 4)) . 'assets/js/admin/database-cleanup.js'),
            ['jquery'],
            '1.0.0',
            true
        );
    ...
    public static function render_page(): void
    {
        ...
        ?>
        <div class="wrap">
            <h1><?php esc_html_e('Database Cleanup & Optimization', 'mhm-rentiva'); ?></h1>
