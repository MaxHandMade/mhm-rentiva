...
final class Export
{
	...
	public static function register(): void
	{
		...
		add_action('restrict_manage_posts', array(self::class, 'render_filters'));
		// Export handler
	...
	private static function build_query_args_from_request(string $post_type): array
	{
		...
		$dq = array();
		if ($date_from) {
			$dq['after'] = $date_from . ' 00:00:00';
		}
		if ($date_to) {
			$dq['before'] = $date_to . ' 23:59:59';
		}
		if (! empty($dq)) {
			$dq['inclusive']    = true;
	...
	private static function render_export_history(): void
	{
		...
		if (! empty($export_history)) {
			...
			foreach ($export_history as $export) {
				...
				echo '<tr>';
				echo '<td>' . esc_html(gmdate('d.m.Y H:i', strtotime($export['date']))) . '</td>';
	...
	private static function log_export_activity(string $post_type, string $format, array $args, int $exported_count = 0): void
	{
		...
		$export_history   = get_transient('mhm_rentiva_export_history') ?: array();
	...
	{
		return get_transient('mhm_rentiva_export_history') ?: array();
	}
