...
final class LogColumns
{
    ...
    {
        add_filter('manage_edit-' . PostType::TYPE . '_columns', [self::class, 'columns']);
        add_action('manage_' . PostType::TYPE . '_posts_custom_column', [self::class, 'render'], 10, 2);
        add_action('restrict_manage_posts', [self::class, 'filters']);
        add_action('pre_get_posts', [self::class, 'handle_filters']);
        add_filter('manage_edit-' . PostType::TYPE . '_sortable_columns', [self::class, 'sortable']);
    }
    ...
    public static function columns(array $cols): array
    {
        ...
        foreach ($cols as $k => $v) {
            if (in_array($k, ['cb', 'title', 'date'], true)) {
                $new[$k] = $v;
            }
        }
    ...
    {
        global $typenow;
        if ($typenow !== PostType::TYPE) return;
    ...
    public static function handle_filters(\WP_Query $q): void
    {
        ...
        if ($screen && $screen->post_type !== PostType::TYPE) return;
        $pt = isset($_GET['post_type']) ? self::sanitize_text_field_safe(wp_unslash($_GET['post_type'])) : '';
        if ($pt && $pt !== PostType::TYPE) return;

        $metaQuery = (array) $q->get('meta_query');
        if (empty($metaQuery)) $metaQuery = ['relation' => 'AND'];
