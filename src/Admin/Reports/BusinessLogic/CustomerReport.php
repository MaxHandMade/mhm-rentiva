...
final class CustomerReport
{
        ...
        if ($data === false) {
            // ✅ OPTIMIZED QUERY - Performance increase with pivot technique
                ...
                "SELECT
                 ...
                 LIMIT 100",
                __('Unknown', 'mhm-rentiva'), $start_date, $end_date
            ));
            ...
            $regional_data = $wpdb->get_results($wpdb->prepare(
                "SELECT
            ...
            // Customer lifecycle
            $customer_lifecycle = [
                'new_customers' => count(array_filter($customers, function($customer) use ($start_date, $end_date) {
            ...
            foreach ($customers as &$customer) {
                ...
                $customer->last_booking_date = date('d.m.Y', strtotime($customer->last_booking_date));
                $customer->first_booking_date = date('d.m.Y', strtotime($customer->first_booking_date));
            }
    ...
    public static function get_customer_details(string $email, string $start_date, string $end_date): array
    {
        ...
        foreach ($bookings as &$booking) {
            ...
            $booking->start_date_formatted = date('d.m.Y', $booking->start_date);
            $booking->end_date_formatted = date('d.m.Y', $booking->end_date);
        }
