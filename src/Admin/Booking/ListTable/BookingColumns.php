...
final class BookingColumns {
	...
	public static function render( string $column, int $post_id ): void {
		switch ( $column ) {
			...
			case 'mhm_booking_type':
				...
				if ( $is_manual === 'manual' ) {
					echo '<span class="booking-type manual">' . esc_html__( 'Manual', 'mhm-rentiva' ) . '</span>';
				} else {
					// Default to online
					echo '<span class="booking-type online">' . esc_html__( 'Online', 'mhm-rentiva' ) . '</span>';
				}
				break;
		}
	}
	...
	private static function calculate_revenue_trend( int $trend_range_days ): float {
		...
		$previous_period_start = gmdate( 'Y-m-01', strtotime( "-{$trend_range_days} days" ) );
		$previous_period_end   = gmdate( 'Y-m-d', strtotime( '-1 day' ) );
