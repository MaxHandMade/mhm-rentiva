...
final class ManualBookingMetaBox extends AbstractMetaBox {
			...
			wp_localize_script(
				...
				array(
					...
					'text'     => array(
						...
						'addonTotal'         => __('Add-on Services Total', 'mhm-rentiva'),
					),
				)
			);
		}
	...
	public static function render(\WP_Post $post, array $args = array()): void
	{
		...
		foreach ($vehicles as $vehicle) {
			if (\MHMRentiva\Admin\Vehicle\Helpers\VehicleDataHelper::get_status($vehicle->ID) === 'active') {
		...
		// ── Card 3: Additional Services (only if addons exist) ──
		if ( ! empty( $available_addons ) ) {
			...
			foreach ( $available_addons as $addon ) {
				...
				echo '<input type="checkbox" name="selected_addons[]" value="' . esc_attr( $addon['id'] ) . '" class="mhm-addon-checkbox" data-price="' . esc_attr( $addon['price'] ) . '"' . checked( $addon['required'], true, false ) . ( $addon['required'] ? ' disabled="disabled"' : '' ) . '>';
			...
			// mhm-hidden instead of style="display:none"
		...
		// Price Calculation — mhm-hidden instead of style="display:none"
		...
		// Actions — mhm-hidden on Create button instead of style="display:none"
	...
	public static function ajax_create_booking(): void
	{
		...
		wp_send_json_success(
			array(
				...
				'message'      => esc_html__('Booking created successfully.', 'mhm-rentiva'),
