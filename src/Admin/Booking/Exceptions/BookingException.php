...
final class BookingException extends MHMException {
	 ...
	 * @return static
	 */
	public static function notFound( int $booking_id, array $context = array() ): self {
		return new self(
			/* translators: %d placeholder. */
			sprintf( __( 'Booking #%d not found.', 'mhm-rentiva' ), $booking_id ),
			self::CODE_NOT_FOUND,
			null,
			array_merge( $context, array( 'booking_id' => $booking_id ) )
		);
	}
	 ...
	 * @return static
	 */
	public static function invalidStatus( string $current_status, string $target_status, array $context = array() ): self {
		return new self(
	 ...
	 * @return static
	 */
	public static function conflict( string $reason, array $context = array() ): self {
		return new self(
	 ...
	 * @return static
	 */
	public static function unauthorized( int $user_id, array $context = array() ): self {
		return new self(
			/* translators: %d placeholder. */
			sprintf( __( 'User #%d is unauthorized for this booking.', 'mhm-rentiva' ), $user_id ),
			self::CODE_UNAUTHORIZED,
			null,
			array_merge( $context, array( 'user_id' => $user_id ) )
		);
	}
	 ...
	 * @return static
	 */
	public static function validationFailed( array $errors, array $context = array() ): self {
		return new self(
			/* translators: %s placeholder. */
			sprintf( __( 'Validation failed: %s', 'mhm-rentiva' ), implode( ', ', $errors ) ),
			self::CODE_VALIDATION_FAILED,
			null,
			array_merge( $context, array( 'validation_errors' => $errors ) )
		);
	}
	 ...
	 * @return static
	 */
	public static function paymentFailed( string $reason, array $context = array() ): self {
		return new self(
			/* translators: %s placeholder. */
			sprintf( __( 'Payment failed: %s', 'mhm-rentiva' ), $reason ),
			self::CODE_PAYMENT_FAILED,
			null,
			array_merge( $context, array( 'payment_reason' => $reason ) )
		);
	}
}
