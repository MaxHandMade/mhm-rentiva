...
final class AddonBooking
{
    ...
    {
        add_action('mhm_rentiva_booking_form_after_vehicle', [self::class, 'render_addon_checkboxes']);
        add_filter('mhm_rentiva_booking_validation', [self::class, 'validate_addon_selection'], 10, 2);
        add_action('mhm_rentiva_booking_summary_addons', [self::class, 'render_booking_summary_addons']);
        add_action('mhm_rentiva_admin_booking_meta_addons', [self::class, 'render_admin_booking_meta']);
        add_action('wp_enqueue_scripts', [self::class, 'enqueue_addon_scripts']);
    }
    ...
    public static function render_addon_checkboxes(): void
    {
        ...
        foreach ($addons as $addon) {
            ...
            echo '<input type="checkbox" name="selected_addons[]" value="' . esc_attr($addon['id']) . '" ' . $checked . ' class="addon-checkbox" data-price="' . esc_attr($addon['price']) . '">';
            echo '<span class="addon-info">';
            echo '<span class="addon-title">' . esc_html($addon['title']) . $required_text . '</span>';
    ...
    {
        $selected_addons = get_post_meta($booking_id, 'mhm_selected_addons', true);
        $addon_details = get_post_meta($booking_id, 'mhm_addon_details', true);
        $addon_total = get_post_meta($booking_id, 'mhm_addon_total', true);
    ...
    public static function get_booking_addons(int $booking_id): array
    {
        $selected_addons = get_post_meta($booking_id, 'mhm_selected_addons', true);
        $addon_details = get_post_meta($booking_id, 'mhm_addon_details', true);
        $addon_total = (float) get_post_meta($booking_id, 'mhm_addon_total', true);
    ...
    public static function enqueue_addon_scripts(): void
    {
        ...
        wp_localize_script('mhm-rentiva-addons', 'mhmRentivaAddons', [
