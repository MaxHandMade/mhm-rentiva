...
/**
 * ✅ CUSTOMERS PERFORMANCE OPTIMIZER
 * 
...
final class CustomersOptimizer
{
    ...
    public static function get_customers_optimized(int $page = 1, int $per_page = 20, string $search = ''): array
    {
        ...
        if (!empty($search)) {
            $where_clause .= " AND (u.display_name LIKE %s OR u.user_email LIKE %s)";
        ...
        if (!empty($search)) {
            $total = $wpdb->get_var($wpdb->prepare($count_query, $search_term, $search_term));
        } else {
        ...
        foreach ($results as $result) {
            $customers[] = [
                ...
                'last_booking' => $result->last_booking ? date('d.m.Y', strtotime($result->last_booking)) : '-',
                'created_date' => $result->created_date ? date('d.m.Y', strtotime($result->created_date)) : '-',
                'currency' => $currency
            ];
        }
    ...
    public static function get_customer_stats_optimized(): array
    {
        ...
        $query = "
            SELECT 
                ...
                COUNT(DISTINCT CASE 
                    WHEN u.user_registered >= DATE_FORMAT(NOW(), '%Y-%m-01') 
                    THEN u.ID 
    ...
    public static function get_customer_details_optimized(int $customer_id): ?array
    {
        ...
        $customer_data = [
            ...
            'phone' => $result->phone ?: '-',
            'address' => $result->address ?: '-',
            'registered' => date('d.m.Y', strtotime($result->user_registered)),
            'booking_count' => (int) $result->booking_count,
            'total_spent' => number_format((float) $result->total_spent, 2, ',', '.'),
            'currency' => $currency,
            'last_booking' => $result->last_booking ? date('d.m.Y H:i', strtotime($result->last_booking)) : '-',
            'first_booking' => $result->first_booking ? date('d.m.Y H:i', strtotime($result->first_booking)) : '-'
        ];
