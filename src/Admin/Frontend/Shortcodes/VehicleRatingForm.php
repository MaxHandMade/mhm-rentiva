...
final class VehicleRatingForm extends AbstractShortcode {
	...
	public static function register(): void {
		...
		add_action( 'wp_ajax_mhm_rentiva_get_vehicle_rating', array( self::class, 'ajax_get_vehicle_rating' ) );
		add_action( 'wp_ajax_nopriv_mhm_rentiva_get_vehicle_rating', array( self::class, 'ajax_get_vehicle_rating' ) );
		add_action( 'wp_ajax_mhm_rentiva_get_vehicle_rating_list', array( self::class, 'ajax_get_vehicle_ratings' ) );
		add_action( 'wp_ajax_nopriv_mhm_rentiva_get_vehicle_rating_list', array( self::class, 'ajax_get_vehicle_ratings' ) );
		...
		add_action( 'wp_ajax_mhm_rentiva_delete_comment', array( self::class, 'ajax_delete_comment' ) );
		add_action( 'wp_ajax_nopriv_mhm_rentiva_delete_comment', array( self::class, 'ajax_delete_comment' ) );
	}
	...
	public static function get_user_rating( int $vehicle_id ): ?array {
		...
		$comments = get_comments(
			array(
				'post_id'    => $vehicle_id,
				'user_id'    => $user_id,
				'status'     => array( 'approve', 'pending' ), // Both approved and pending
				'number'     => 1,
				'meta_query' => array(
					array(
						'key'     => 'mhm_rating',
	...
	public static function ajax_submit_rating(): void {
		try {
			...
			if ( ! $require_login && $allow_guest_comments ) {
				...
				if ( $user_id < 0 ) {
					wp_send_json_error( array( 'message' => __( 'Invalid user session.', 'mhm-rentiva' ) ) );
				}
			...
			if ( ! is_user_logged_in() && $allow_guest_comments ) {
				...
				if ( empty( $guest_name ) || empty( $guest_email ) ) {
					...
					wp_send_json_error( array( 'message' => __( 'Name and email are required for guest comments.', 'mhm-rentiva' ) ) );
				}

				if ( ! is_email( $guest_email ) ) {
					error_log( 'AJAX Submit Rating - Invalid email format: ' . $guest_email );
					wp_send_json_error( array( 'message' => __( 'Please enter a valid email address.', 'mhm-rentiva' ) ) );
				}
			...
			if ( $rating < 1 || $rating > 5 ) {
				wp_send_json_error( array( 'message' => __( 'Please select a valid rating.', 'mhm-rentiva' ) ) );
			}

			// SPAM KORUMASI
			if ( ! self::check_spam_protection( $user_id, $vehicle_id, $comment ) ) {
				wp_send_json_error( array( 'message' => __( 'Spam protection triggered. Please try again later.', 'mhm-rentiva' ) ) );
			}

			// RATE LIMITING - Is user submitting too quickly?
			if ( ! self::check_rate_limiting( $user_id ) ) {
				wp_send_json_error( array( 'message' => __( 'Too many requests. Please wait before submitting another rating.', 'mhm-rentiva' ) ) );
			}
			...
			if ( $existing_rating ) {
				...
				if ( $comment_id ) {
					...
					wp_send_json_error( array( 'message' => __( 'Failed to create rating.', 'mhm-rentiva' ) ) );
				}
			}
			...
			wp_send_json_error( array( 'message' => __( 'An error occurred while submitting rating.', 'mhm-rentiva' ) ) );
		}
	}
	...
	public static function ajax_get_vehicle_rating(): void {
		try {
			...
			wp_send_json_error( array( 'message' => __( 'An error occurred while retrieving rating.', 'mhm-rentiva' ) ) );
		}
	}
	...
	public static function ajax_get_vehicle_ratings(): void {
		try {
			...
			*/
			...
			wp_send_json_error( array( 'message' => __( 'An error occurred while retrieving ratings.', 'mhm-rentiva' ) ) );
		}
	}
	...
	public static function ajax_delete_rating(): void {
		try {
			...
			if ( $require_login && ! is_user_logged_in() ) {
				wp_send_json_error( array( 'message' => __( 'You must be logged in to delete rating.', 'mhm-rentiva' ) ) );
			}
			...
			if ( ! $existing_rating ) {
				wp_send_json_error( array( 'message' => __( 'No rating found to delete.', 'mhm-rentiva' ) ) );
			}
			...
			wp_send_json_error( array( 'message' => __( 'An error occurred while deleting rating.', 'mhm-rentiva' ) ) );
		}
	}
	...
	public static function ajax_delete_comment(): void {
		...
		if ( $require_login && ! is_user_logged_in() ) {
			wp_send_json_error( array( 'message' => __( 'You must be logged in to delete comments.', 'mhm-rentiva' ) ) );
		}

		$comment_id = intval( $_POST['comment_id'] ?? 0 );
		if ( $comment_id <= 0 ) {
			wp_send_json_error( array( 'message' => __( 'Invalid comment ID.', 'mhm-rentiva' ) ) );
		}
		...
		if ( $comment->user_id != get_current_user_id() ) {
			wp_send_json_error( array( 'message' => __( 'You can only delete your own comments.', 'mhm-rentiva' ) ) );
		}

		// Delete comment
		$deleted = wp_delete_comment( $comment_id, true );
		if ( ! $deleted ) {
			wp_send_json_error( array( 'message' => __( 'Failed to delete comment.', 'mhm-rentiva' ) ) );
		}
		...
		wp_send_json_success( array( 'message' => __( 'Comment deleted successfully.', 'mhm-rentiva' ) ) );
	}
}
