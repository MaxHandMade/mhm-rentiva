	...
	public static function ajax_booking_form(): void {
		try {
			...
			if ( $end_date <= $start_date ) {
				wp_send_json_error( array( 'message' => __( 'Return date must be after pickup date.', 'mhm-rentiva' ) ) );
			}
			...
			if ( $days < $min_days ) {
				wp_send_json_error(
					array(
						/* translators: %d: minimum days */
						'message' => sprintf( esc_html__( 'Minimum rental period is %d days.', 'mhm-rentiva' ), $min_days ),
						'code'    => 'min_days_error',
					)
				);
			}
			...
			if ( $max_days > 0 && $days > $max_days ) {
				wp_send_json_error(
					array(
						/* translators: %d: maximum days */
						'message' => sprintf( esc_html__( 'Maximum rental period is %d days.', 'mhm-rentiva' ), $max_days ),
						'code'    => 'max_days_error',
					)
				);
			}
		...
		try {
			...
			if ( $days < $min_rental_days ) {
				wp_send_json_error(
					array(
						/* translators: %d: number of days */
						'message' => sprintf( __( 'Minimum rental period is %d days.', 'mhm-rentiva' ), $min_rental_days ),
					)
				);
			}
			...
			if ( $days > $max_rental_days ) {
				wp_send_json_error(
					array(
						/* translators: %d: number of days */
						'message' => sprintf( __( 'Maximum rental period is %d days.', 'mhm-rentiva' ), $max_rental_days ),
					)
				);
			}
