...
final class AvailabilityCalendar extends AbstractShortcode {
	...
	public static function register(): void {
		...
		// AJAX handlers
		add_action( 'wp_ajax_mhm_rentiva_availability_data', array( self::class, 'ajax_availability_data' ) );
		add_action( 'wp_ajax_nopriv_mhm_rentiva_availability_data', array( self::class, 'ajax_availability_data' ) );

		add_action( 'wp_ajax_mhm_rentiva_availability_pricing', array( self::class, 'ajax_availability_pricing' ) );
		add_action( 'wp_ajax_nopriv_mhm_rentiva_availability_pricing', array( self::class, 'ajax_availability_pricing' ) );

		add_action( 'wp_ajax_mhm_rentiva_load_booking_form', array( self::class, 'ajax_load_booking_form' ) );
		add_action( 'wp_ajax_nopriv_mhm_rentiva_load_booking_form', array( self::class, 'ajax_load_booking_form' ) );
			...
			if ( self::asset_exists( $js_file ) ) {
				...
				wp_localize_script(
					...
					array(
						...
						'strings'        => self::get_localized_strings(), // Inject strings directly
					)
				);
	...
	public static function ajax_availability_data(): void {
		...
		try {
			...
			if ( ! $vehicle_id ) {
				wp_send_json_error( array( 'message' => __( 'Vehicle ID is required.', 'mhm-rentiva' ) ) );
			}
			...
			wp_send_json_error( array( 'message' => __( 'An error occurred while retrieving data.', 'mhm-rentiva' ) ) );
		}
	}
	...
	public static function ajax_availability_pricing(): void {
		...
		try {
			...
			wp_send_json_error( array( 'message' => __( 'An error occurred while retrieving pricing data.', 'mhm-rentiva' ) ) );
		}
	}
	...
	public static function ajax_get_vehicle_info(): void {
		try {
			...
			if ( $fuel_type ) {
				$specs['fuel'] = \MHMRentiva\Admin\Vehicle\Helpers\VehicleDataHelper::get_fuel_type_label( $fuel_type );
			}
			if ( $transmission ) {
				$specs['transmission'] = \MHMRentiva\Admin\Vehicle\Helpers\VehicleDataHelper::get_transmission_label( $transmission );
			}
			if ( $year ) {
				$specs['year'] = $year;
			}
			if ( $mileage ) {
				/* translators: %s: mileage value */
				$specs['mileage'] = sprintf( __( '%s miles', 'mhm-rentiva' ), $mileage );
			}
			if ( $seats ) {
				/* translators: %d: number of seats */
				$specs['seats'] = sprintf( __( '%d people', 'mhm-rentiva' ), $seats );
			}
			...
			wp_send_json_error( array( 'message' => __( 'An error occurred while retrieving vehicle information.', 'mhm-rentiva' ) ) );
		}
	}
	...
	public static function ajax_load_booking_form(): void {
		try {
			...
			if ( $vehicle_id <= 0 || empty( $start_date ) || empty( $end_date ) ) {
				wp_send_json_error( array( 'message' => __( 'Invalid parameters.', 'mhm-rentiva' ) ) );
			}
			...
			wp_send_json_error( array( 'message' => __( 'An error occurred while loading the booking form.', 'mhm-rentiva' ) ) );
		}
	}
