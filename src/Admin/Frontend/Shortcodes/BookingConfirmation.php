...
final class BookingConfirmation extends AbstractShortcode {
	...
	protected static function prepare_template_data( array $atts ): array {
		...
		if ( $order_id && function_exists( 'wc_get_order' ) ) {
			...
			if ( $order instanceof \WC_Order ) {
				...
				$billing_first_name = method_exists( $order, 'get_billing_first_name' ) ? $order->get_billing_first_name() : '';
				$billing_last_name  = method_exists( $order, 'get_billing_last_name' ) ? $order->get_billing_last_name() : '';
	...
	public static function get_confirmation_url( int $booking_id ): string {
		...
		return add_query_arg( array( 'booking_id' => $booking_id ), $confirmation_url );
	}
