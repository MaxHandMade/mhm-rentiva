...
// phpcs:disable WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- Vehicle list shortcode uses bounded filter/sort and aggregate lookup queries.
...
final class VehiclesList extends AbstractShortcode {
	 ...
	 */
	private static ?string $booking_url_cache = null;
	...
	public static function get_vehicle_data_for_shortcode(int $vehicle_id, array $atts): array
	{
		...
		// Prepare Image Data
		$image_url      = self::get_vehicle_image($vehicle_id, $atts['image_size'] ?? 'large');
		$image_thumb_id = (int) get_post_thumbnail_id($vehicle_id);
		$image_data     = array(
			'url' => $image_url,
	...
	{
		if (null !== self::$booking_url_cache) {
			return self::$booking_url_cache;
		}
		...
		self::$booking_url_cache = $url ?: home_url('/');
		return self::$booking_url_cache;
	}
