...
final class SystemInfo
{
    ...
    private static function safe_get_system_info(): array
    {
        try {
            ...
            return [
                'error' => __('Error occurred while getting system information.', 'mhm-rentiva'),
                'wordpress' => [],
                'php' => [],
                'plugin' => [],
                'database' => [],
    ...
    private static function get_plugin_info(): array
    {
        return [
            ...
            'install_date' => get_option('mhm_rentiva_install_date', __('Unknown', 'mhm-rentiva')),
            'last_update' => get_option('mhm_rentiva_last_update', __('Unknown', 'mhm-rentiva')),
            'license_status' => Mode::isPro() ? __('Pro Active', 'mhm-rentiva') : __('Lite Version', 'mhm-rentiva'),
        ];
    }
    ...
    private static function get_plugin_file_size(): string
    {
        ...
        try {
            ...
            return __('Could not calculate', 'mhm-rentiva');
        }
    }
    ...
    private static function get_plugin_tables(): array
    {
        ...
        foreach ($plugin_tables as $key => $table_name) {
            try {
                ...
                if ($exists) {
                    if ($key === 'mhm_payment_log') {
                        $count = $wpdb->get_var("SELECT COUNT(*) FROM {$table_name}");
                    } else {
                        $post_type = $key === 'vehicle_booking' ? 'vehicle_booking' : ($key === 'mhm_message' ? 'mhm_message' : '');
