...
// phpcs:disable WordPress.DB.SlowDBQuery.slow_db_query_meta_query,WordPress.DB.SlowDBQuery.slow_db_query_meta_key,WordPress.DB.SlowDBQuery.slow_db_query_meta_value,WordPress.DB.SlowDBQuery.slow_db_query_tax_query,WordPress.DB.DirectDatabaseQuery.DirectQuery,WordPress.DB.DirectDatabaseQuery.NoCaching -- GDPR routines intentionally perform bounded retention/export queries.
...
final class GDPRManager {
	...
	public static function init(): void {
		...
		add_action( 'wp_ajax_mhm_rentiva_consent_accept', array( self::class, 'ajax_consent_accept' ) );
	}
	...
	public static function enqueue_gdpr_scripts(): void {
		...
		wp_enqueue_script( 'mhm-gdpr', MHM_RENTIVA_PLUGIN_URL . 'assets/js/gdpr.js', array( 'jquery' ), MHM_RENTIVA_VERSION, true );
		wp_localize_script(
			'mhm-gdpr',
			'mhm_gdpr',
			array(
	...
	/**
	 * AJAX handler for accepting GDPR consent (popup notice)
	 */
	public static function ajax_consent_accept(): void {
		if ( ! is_user_logged_in() ) {
			wp_send_json_error( array( 'message' => esc_html__( 'Not logged in.', 'mhm-rentiva' ) ) );
			return;
		}
		...
		self::record_user_consent( get_current_user_id() );
		wp_send_json_success( array( 'message' => esc_html__( 'Consent recorded.', 'mhm-rentiva' ) ) );
	}
