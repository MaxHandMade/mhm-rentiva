...
final class GDPRManager
{
    ...
    {
        add_action('init', [self::class, 'check_gdpr_compliance']);
        add_action('wp_ajax_mhm_rentiva_data_export', [self::class, 'ajax_data_export']);
        add_action('wp_ajax_mhm_rentiva_data_deletion', [self::class, 'ajax_data_deletion']);
        add_action('wp_ajax_mhm_rentiva_consent_withdrawal', [self::class, 'ajax_consent_withdrawal']);
    }
    ...
    public static function check_booking_consent(): void
    {
        ...
        if ($consent_given !== '1') {
            wp_die(__('You must provide consent for data processing before making a booking.', 'mhm-rentiva'));
        }
    }
    ...
    public static function enqueue_gdpr_scripts(): void
    {
        ...
        wp_enqueue_script('mhm-gdpr', plugin_dir_url(__FILE__) . '../assets/js/gdpr.js', ['jquery'], '1.0.0', true);
        wp_localize_script('mhm-gdpr', 'mhm_gdpr', [
    ...
    public static function ajax_data_deletion(): void
    {
        ...
        if (self::delete_user_data($user_id)) {
            ...
            wp_send_json_error(['message' => __('Failed to delete data.', 'mhm-rentiva')]);
        }
    }
    ...
    public static function ajax_consent_withdrawal(): void
    {
        ...
        wp_send_json_success(['message' => __('Consent withdrawn successfully.', 'mhm-rentiva')]);
    }
