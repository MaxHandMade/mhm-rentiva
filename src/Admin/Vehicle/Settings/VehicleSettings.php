...
final class VehicleSettings
{
    ...
    public static function register(): void
    {
        ...
        // Reset Settings
        add_action('wp_ajax_mhm_reset_vehicle_settings', [self::class, 'ajax_reset_settings']);
    }
...
?>
    ...
    private static function render_display_tab(): void
    {
        ...
        $selected_comparison_fields = $settings['comparison_fields'] ?? [];
        $available_comparison_fields = self::get_comparison_available_fields();
        $show_defaults = empty($selected_comparison_fields);

        ?>
            <form method="post" action="<?php echo admin_url('admin-ajax.php'); ?>" id="vehicle-display-settings-form">

                <div class="mhm-settings-section">
                    <h2><?php echo esc_html__('Visible Card Items', 'mhm-rentiva'); ?></h2>
                    <div class="mhm-card-fields-wrapper">
                        <input type="hidden" id="mhm-vehicle-card-fields-input" name="mhm_rentiva_vehicle_card_fields" value="<?php echo $hidden_value; ?>" />
                                ...
                                <ul id="mhm-card-fields-selected" class="mhm-card-fields-list" style="min-height: 50px; border: 1px dashed #ccc; padding: 10px;">
                                    <?php if (!empty($selected_items)): ?>
                                        <?php foreach ($selected_items as $item): ?>
                                            <?php echo self::render_card_field_list_item($item['type'], $item['key'], $item['label'], true); ?>
                                        <?php endforeach; ?>
                                    <?php endif; ?>
                                </ul>
                            </div>
                                ...
                                <ul id="mhm-card-fields-available" class="mhm-card-fields-list" style="min-height: 50px; border: 1px dashed #ccc; padding: 10px;">
                                    <?php if (!empty($available_items)): ?>
                                        <?php foreach ($available_items as $item): ?>
                                            <?php echo self::render_card_field_list_item($item['type'], $item['key'], $item['label'], false); ?>
                                        <?php endforeach; ?>
                                    <?php endif; ?>
                                </ul>
                            </div>
                                            ...
                                            <label class="mhm-field-item" style="display: flex; align-items: center; padding: 5px; background: white; border-radius: 3px;">
                                                <input type="checkbox"
                                                    ...
                                                    if ($show_defaults) {
                                                        ...
                                                        $is_checked = in_array($field_key, $selected_comparison_fields[$category] ?? []);
                                                    }
                                                    checked($is_checked);
                                                    ?>>
    ...
    {
        $taxonomy_features = [];
        $taxonomies = ['mhm_rentiva_feature', 'vehicle_feature', 'vehicle_features'];
    ...
    {
        $taxonomy_equipment = [];
        $taxonomies = ['mhm_rentiva_equipment', 'vehicle_equipment'];
        foreach ($taxonomies as $tax) {
            if (taxonomy_exists($tax)) {
                $terms = get_terms(['taxonomy' => $tax, 'hide_empty' => false]);
                if (!empty($terms) && !is_wp_error($terms)) {
    ...
    public static function ajax_save_settings(): void
    {
        ...
        if (isset($_POST['sub_action']) && $_POST['sub_action'] === 'save_display_settings') {
            ...
            $comparison_fields = $_POST['comparison_fields'] ?? [];
    ...
    public static function ajax_remove_custom_field(): void
    {
        ...
        if ($field_type === 'details') {
            ...
            if (isset($current_details[$field_key])) {
                ...
                // Clean meta
                global $wpdb;
                $wpdb->delete($wpdb->postmeta, ['meta_key' => '_mhm_rentiva_' . $field_key]);
    ...
    public static function ajax_add_custom_field(): void
    {
        ...
        if ($field_type === 'details') {
            ...
            if (!empty($_POST['type'])) {
                $field_meta = get_option('mhm_custom_field_meta', []);
                $field_meta[$field_key] = [
                    'type' => self::sanitize_text_field_safe($_POST['type']),
    ...
    public static function ajax_reset_settings(): void
    {
        ...
        wp_send_json_success(['message' => __('Settings reset to defaults.', 'mhm-rentiva')]);
    }
}
