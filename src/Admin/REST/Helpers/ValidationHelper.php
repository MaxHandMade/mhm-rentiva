...
final class ValidationHelper
{
    ...
    public static function validateBookingForPayment(
        ...
        array $allowed_statuses = ['unpaid', 'failed', '']
    ): bool {
        if ($booking_id <= 0) {
            return false;
        }
    ...
    public static function validateBookingDetailed(
        ...
        string $payment_meta_key = '_mhm_payment_status',
        array $allowed_statuses = ['pending', 'confirmed'],
        array $blocked_payment_statuses = ['paid', 'refunded']
    ): array {
        if ($booking_id <= 0) {
            return ['valid' => false, 'error' => 'invalid_booking_id'];
        }

        $post = get_post($booking_id);
        if (!$post || $post->post_type !== $post_type) {
            return ['valid' => false, 'error' => 'booking_not_found'];
        }
    ...
    public static function validateBookingForRefund(
        ...
        if ($paymentStatus !== 'paid') {
            return ['valid' => false, 'error' => 'not_paid'];
        }
