...
final class ErrorHandler
{
    ...
    {
        add_action('rest_api_init', [self::class, 'register_error_handlers']);
        add_filter('rest_pre_serve_request', [self::class, 'handle_rest_errors'], 10, 4);
    }
    ...
    public static function register_error_handlers(): void
    {
        ...
        add_filter('rest_prepare_vehicle', [self::class, 'prepare_vehicle_response'], 10, 3);

        // General REST API error handling
        add_filter('rest_request_before_callbacks', [self::class, 'before_rest_callbacks'], 10, 3);
        add_filter('rest_request_after_callbacks', [self::class, 'after_rest_callbacks'], 10, 3);
    }
    ...
    public static function prepare_vehicle_response($response, $post, $request)
    {
        ...
        try {
            ...
            $meta_data = [
                'price_per_day' => get_post_meta($post->ID, '_mhm_rentiva_price_per_day', true),
                'seats' => get_post_meta($post->ID, '_mhm_rentiva_seats', true),
                ...
                'available' => get_post_meta($post->ID, '_mhm_vehicle_availability', true) === 'active',
            ];
            ...
            $response->data['mhm_vehicle_meta'] = [];
        }
