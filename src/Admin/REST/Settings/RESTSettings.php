...
final class RESTSettings
{
    ...
    {
        return self::get_setting('development', []);
    }
    ...
    public static function sanitize_settings(array $input): array
    {
        ...
        if (isset($input['security']) && is_array($input['security'])) {
            ...
            $ip_whitelist = $to_array($sec['ip_whitelist'] ?? []);
            $ip_blacklist = $to_array($sec['ip_blacklist'] ?? []);
    ...
    public static function render_settings_section(): void
    {
        ...
        echo '<textarea id="rest_ip_whitelist" name="mhm_rentiva_rest_settings[security][ip_whitelist]" rows="2" cols="50" class="regular-text" placeholder="1.2.3.4, 5.6.7.8">' . esc_textarea(implode(', ', (array)($sec['ip_whitelist'] ?? []))) . '</textarea>';
    ...
    {
        check_ajax_referer('mhm_rest_api_keys_nonce', 'nonce');
        if (!current_user_can('manage_options')) wp_send_json_error();
        $name = sanitize_text_field($_POST['name'] ?? '');
        $perms = (array)($_POST['permissions'] ?? ['read']);
        $key = \MHMRentiva\Admin\REST\APIKeyManager::create_api_key($name, $perms);
        $key ? wp_send_json_success(['key' => $key]) : wp_send_json_error();
    }
    ...
    public static function ajax_list_api_keys(): void
    {
        check_ajax_referer('mhm_rest_api_keys_nonce', 'nonce');
        if (!current_user_can('manage_options')) wp_send_json_error();
        wp_send_json_success(['keys' => \MHMRentiva\Admin\REST\APIKeyManager::list_api_keys()]);
    }
