...
final class WooCommerceBridge implements PaymentGatewayInterface
{
	...
	public static function register(): void
	{
		...
		add_filter('woocommerce_cart_item_thumbnail', array(self::class, 'display_vehicle_image'), 10, 3);
	...
	public static function add_booking_to_cart(int $booking_id, float $amount): bool
	{
		...
		try {
			...
			AdvancedLogger::error('Failed to add booking to cart', array('booking_id' => $booking_id, 'exception' => $e->getMessage()), AdvancedLogger::CATEGORY_PAYMENT);
			return false;
		}
	}
	...
	public static function get_item_data($item_data, $cart_item)
	{
		...
		elseif ($booking_data && $vehicle_id) {
			...
			if (! $hide_image) {
				...
				if ($vehicle_image) {
					$item_data[] = array(
						'key'   => esc_html__('Vehicle Image', 'mhm-rentiva'),
	...
	private static function create_booking_from_data(array $booking_data, int $order_id): ?int
	{
		...
		if (! $start_ts || ! $end_ts) {
			AdvancedLogger::error('Invalid dates in booking data', array('order_id' => $order_id, 'booking_data' => array_intersect_key($booking_data, array_flip(['pickup_date', 'dropoff_date']))), AdvancedLogger::CATEGORY_BOOKING);
			return null;
		}
		...
		if (\MHMRentiva\Admin\Booking\Helpers\Util::has_overlap_locked($booking_data['vehicle_id'], $start_ts, $end_ts)) {
			AdvancedLogger::error('Cannot create booking - vehicle already booked for selected dates', array('order_id' => $order_id, 'vehicle_id' => $booking_data['vehicle_id']), AdvancedLogger::CATEGORY_BOOKING);
		...
		if (is_wp_error($booking_id)) {
			AdvancedLogger::error('Failed to create booking from order', array('order_id' => $order_id, 'error' => $booking_id->get_error_message()), AdvancedLogger::CATEGORY_BOOKING);
			return null;
		}
		...
		if (empty($payment_deadline)) {
			...
			AdvancedLogger::info('Payment deadline was missing for booking, set automatically', array('booking_id' => $booking_id, 'deadline' => $deadline), AdvancedLogger::CATEGORY_BOOKING);
		}
				...
				<label class="mhm-payment-type-option <?php echo $current_payment_type === 'deposit' ? 'selected' : ''; ?>">
					...
					<div class="mhm-payment-type-option-content">
						...
						<div class="mhm-payment-type-option-price">
							<?php echo wp_kses_post(wc_price($total_deposit_amount)); ?>
							<?php if ($total_remaining_amount > 0) : ?>
								<span class="mhm-payment-type-option-price-remaining">
									(<?php echo esc_html__('Remaining:', 'mhm-rentiva'); ?> <?php echo wp_kses_post(wc_price($total_remaining_amount)); ?>)
								</span>
							<?php endif; ?>
						</div>
					</div>
		...
		foreach ($cart->get_cart() as $cart_item) {
			...
			if (isset($cart_item['mhm_booking_data']) && isset($cart_item['mhm_booking_pending']) && $cart_item['mhm_booking_pending']) {
				...
				if (\MHMRentiva\Admin\Booking\Helpers\Util::has_overlap_locked($vehicle_id, $start_ts, $end_ts)) {
					...
					// Log for debugging
					AdvancedLogger::warning('Checkout validation failed - vehicle no longer available', array('vehicle_id' => $vehicle_id, 'dates' => $pickup_date . ' - ' . $dropoff_date), AdvancedLogger::CATEGORY_BOOKING);
				}
			}
			// Check existing booking
			elseif (isset($cart_item['mhm_booking_id'])) {
	...
	public static function handle_order_refunded(int $refund_id, array $args): void
	{
		...
		if (class_exists('\MHMRentiva\Admin\Emails\Notifications\RefundNotifications')) {
			try {
				...
				AdvancedLogger::error('Refund notification error', array('booking_id' => $booking_id, 'exception' => $e->getMessage()), AdvancedLogger::CATEGORY_PAYMENT);
			}
		}
