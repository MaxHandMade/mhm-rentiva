...
final class WafManager
{
    ...
    public static function init(): void
    {
        ...
        add_action('plugins_loaded', [self::class, 'inspect_request'], 1);
    }
    ...
    public static function inspect_request(): void
    {
        ...
        $check_sqli = SettingsCore::get('mhm_rentiva_sql_injection_protection', '0') === '1';
        $check_xss = SettingsCore::get('mhm_rentiva_xss_protection', '0') === '1';
    ...
    private static function block_request(string $reason, string $type, string $key): void
    {
        ...
        if (class_exists('\MHMRentiva\Admin\PostTypes\Logs\AdvancedLogger')) {
            \MHMRentiva\Admin\PostTypes\Logs\AdvancedLogger::warning(
                sprintf(__('WAF Blocked Request: %s in %s[%s]', 'mhm-rentiva'), $reason, $type, $key),
                ['ip' => $_SERVER['REMOTE_ADDR'] ?? 'unknown'],
                'security'
            );
        } else {
            error_log(sprintf('MHM WAF Blocked: %s in %s[%s] IP: %s', $reason, $type, $key, $_SERVER['REMOTE_ADDR'] ?? 'unknown'));
        }
