...
final class PerformanceTest
{
    ...
    public static function test_cache_hit_rate(): array
    {
        ...
        // First write
        \MHMRentiva\Admin\Core\Utilities\CacheManager::set_cache('dashboard_stats', $test_key, ['data' => 'test'], 3600);
        ...
        // Clean up
        \MHMRentiva\Admin\Core\Utilities\CacheManager::clear_cache(['dashboard_stats']);
    ...
    public static function test_asset_loading_time(): array
    {
        ...
        $frontend_css_files = array_merge(
            glob($assets_dir . 'css/frontend/*.css') ?: [],
            glob($assets_dir . 'css/core/*.css') ?: [],
            glob($assets_dir . 'css/components/*.css') ?: []
        );
        ...
        $frontend_js_files = array_merge(
            glob($assets_dir . 'js/frontend/*.js') ?: [],
            glob($assets_dir . 'js/core/*.js') ?: [],
            glob($assets_dir . 'js/components/*.js') ?: []
        );
        ...
        $admin_css_files = glob($assets_dir . 'css/admin/*.css') ?: [];
        $admin_js_files = glob($assets_dir . 'js/admin/*.js') ?: [];
