		...
		<div class="wrap mhm-test-page">
			...
			<?php if ($test_results) : ?>
				<div class="mhm-test-results-container">
					<?php
					// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- The report is internally generated and contains legitimate <style> tags that wp_kses_post would strip.
					echo TestRunner::render_html_report($test_results);
					?>
...
<?php
	...
	public static function handle_download_report(): void
	{
		...
		if ($format === 'html') {
			...
			$filename     = 'mhm-rentiva-test-report-' . gmdate('Y-m-d-H-i-s') . '.json';
		...
		// Send content
		// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Downloading generated report (HTML/JSON).
		echo $content;
		exit;
	}
