...
final class TestRunner
{
    ...
    /**
     * Test sonuçlarını HTML olarak render et
     */
    public static function render_html_report(array $test_results): string
    {
        ...
        <div class="mhm-test-report">
            <h1>🧪 <?php _e('MHM Rentiva - Test Report', 'mhm-rentiva'); ?></h1>

            <!-- Summary -->
            <div class="test-summary test-summary-<?php echo esc_attr($summary['overall_status']); ?>">
                <h2>📊 <?php _e('Summary', 'mhm-rentiva'); ?></h2>
                <div class="summary-stats">
                    <div class="stat">
                        <span class="stat-label"><?php _e('Total Tests:', 'mhm-rentiva'); ?></span>
                        <span class="stat-value"><?php echo esc_html($summary['total_tests']); ?></span>
                    </div>
                    <div class="stat stat-pass">
                        <span class="stat-label"><?php _e('Successful:', 'mhm-rentiva'); ?></span>
                        <span class="stat-value"><?php echo esc_html($summary['passed']); ?></span>
                    </div>
                    <div class="stat stat-fail">
                        <span class="stat-label"><?php _e('Failed:', 'mhm-rentiva'); ?></span>
                        <span class="stat-value"><?php echo esc_html($summary['failed']); ?></span>
                    </div>
                    <div class="stat stat-warning">
                        <span class="stat-label"><?php _e('Warnings:', 'mhm-rentiva'); ?></span>
                        <span class="stat-value"><?php echo esc_html($summary['warnings']); ?></span>
                    </div>
                    <div class="stat">
                        <span class="stat-label"><?php _e('Success Rate:', 'mhm-rentiva'); ?></span>
                ...
                <div class="overall-status">
                    ...
                    ?>
                    <h3><?php echo $status_icon; ?> <?php _e('Overall Status:', 'mhm-rentiva'); ?> <?php echo esc_html($status_text); ?></h3>
                </div>
            </div>
            ...
            <!-- Execution Info -->
            <div class="test-info">
                <p><strong>⏱️ <?php _e('Execution Time:', 'mhm-rentiva'); ?></strong> <?php echo esc_html($test_results['execution_time']); ?> <?php _e('seconds', 'mhm-rentiva'); ?></p>
                <p><strong>📅 <?php _e('Test Date:', 'mhm-rentiva'); ?></strong> <?php echo esc_html($test_results['timestamp']); ?></p>
            </div>

            <style>
                .mhm-test-report { max-width: 1200px; margin: 20px auto; padding: 20px; background: #fff; }
                .test-summary { padding: 20px; margin: 20px 0; background: #f5f5f5; border-radius: 5px; }
                .test-summary-excellent { border-left: 4px solid #46b450; }
                .test-summary-good { border-left: 4px solid #00a0d2; }
                .test-summary-acceptable { border-left: 4px solid #ffb900; }
                .test-summary-poor, .test-summary-fail { border-left: 4px solid #dc3232; }
                .summary-stats { display: grid; grid-template-columns: repeat(auto-fit, minmax(150px, 1fr)); gap: 15px; margin: 15px 0; }
                .stat { padding: 10px; background: #fff; border-radius: 3px; text-align: center; }
                .stat-pass { border-left: 3px solid #46b450; }
                .stat-fail { border-left: 3px solid #dc3232; }
                .stat-warning { border-left: 3px solid #ffb900; }
                ...
                .test-suite { margin: 30px 0; }
                .test-row-pass { background-color: #f0f8f0; }
                .test-row-fail { background-color: #fff0f0; }
                .test-row-warning { background-color: #fffbf0; }
                .test-row-skip { background-color: #f9f9f9; }
                .test-badge { display: inline-block; padding: 4px 10px; border-radius: 3px; font-size: 11px; font-weight: bold; }
                .test-badge-pass { background: #46b450; color: #fff; }
                .test-badge-fail { background: #dc3232; color: #fff; }
                .test-badge-warning { background: #ffb900; color: #000; }
                .test-badge-skip { background: #999; color: #fff; }
                .test-info { margin-top: 30px; padding: 15px; background: #f5f5f5; border-radius: 3px; }
                .overall-status { margin-top: 20px; text-align: center; }
                .overall-status h3 { font-size: 24px; margin: 0; }
            </style>
        </div>
        <?php
        return ob_get_clean();
    ...
    /**
     * Test sonuçlarını JSON olarak dönder
     */
    ...
    public static function save_report_to_file(array $test_results, string $format = 'html'): string
    {
        ...
        $filename = 'test-report-' . date('Y-m-d-H-i-s') . '.' . $format;
