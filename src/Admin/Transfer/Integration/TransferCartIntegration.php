...
final class TransferCartIntegration {
	...
	public static function handle_add_to_cart_ajax(): void
	{
		...
		if ($selected_price !== null && $selected_distance !== null) {
			// Check rentiva_ prefix first, then fallback to mhm_
			$price_per_km = get_post_meta($vehicle_id, '_rentiva_vehicle_price_per_km', true);
			if ($price_per_km === '') {
				$price_per_km = get_post_meta($vehicle_id, '_mhm_vehicle_price_per_km', true);
			}
			$price_per_km = (float) $price_per_km;

			$base_price = get_post_meta($vehicle_id, '_rentiva_vehicle_base_price', true);
			if ($base_price === '') {
				$base_price = get_post_meta($vehicle_id, '_mhm_vehicle_base_price', true);
			}
		...
		$deposit_rate = (int) \MHMRentiva\Admin\Settings\Core\SettingsCore::get('rentiva_transfer_deposit_rate', '20');
