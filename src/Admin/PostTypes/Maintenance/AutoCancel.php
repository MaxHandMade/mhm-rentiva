...
final class AutoCancel
{
	...
	public static function maybe_schedule(): void
	{
		...
		if (! isset($schedules[self::SCHEDULE])) {
			AdvancedLogger::warning('Custom schedule not found', array('schedule' => self::SCHEDULE, 'available' => array_keys($schedules)));
			return;
		}
		...
		$schedules = wp_get_schedules();
		if (! isset($schedules[self::SCHEDULE])) {
			AdvancedLogger::error('Schedule not available when attempting to schedule event', array('schedule' => self::SCHEDULE, 'available' => array_keys($schedules)));
			return;
		}
		...
		if (! isset($schedule_info['interval']) || $schedule_info['interval'] !== 300) {
			AdvancedLogger::error('Schedule has incorrect interval', array('schedule' => self::SCHEDULE, 'interval' => $schedule_info['interval'] ?? 'missing'));
			return;
		}
	...
	private static function direct_schedule_event(): void
	{
		...
		if ($verify_next && $verify_schedule === self::SCHEDULE) {
			AdvancedLogger::info('Successfully scheduled recurring event', array('schedule' => self::SCHEDULE, 'next_run' => wp_date('Y-m-d H:i:s', $verify_next)));
		} else {
			AdvancedLogger::error('Direct schedule failed', array('next' => $verify_next ? wp_date('Y-m-d H:i:s', $verify_next) : 'none', 'schedule' => $verify_schedule ?: 'none'));
		}
	}
}